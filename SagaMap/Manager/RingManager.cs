...
namespace SagaMap.Manager
{
    public class RingManager:Singleton<RingManager>
    {
        ...
        //ECOKEY 換團長
        public void ChangeRingMaster(Ring ring, ActorPC oldmaster, ActorPC newmaster)
        {
            if (ring == null)
                return;

            ring.Leader = newmaster;
            ring.Rights[ring.IndexOf(newmaster)].Value = 29;
            ring.Rights[ring.IndexOf(oldmaster)].Value = 0;
            SagaMap.MapServer.charDB.ChangeRingMaster(ring, oldmaster, newmaster);

            ActorPC[] list = ring.Members.Values.ToArray();
            foreach (ActorPC i in list)
            {
                SagaMap.Packets.Server.SSMG_RING_RIGHT_UPDATE p = new SagaMap.Packets.Server.SSMG_RING_RIGHT_UPDATE();
                p.CharID = oldmaster.CharID;
                p.Right = ring.Rights[ring.IndexOf(oldmaster)].Value;
                if (i.Online)
                    MapClient.FromActorPC(i).netIO.SendPacket(p);

                SagaMap.Packets.Server.SSMG_RING_RIGHT_UPDATE p1 = new SagaMap.Packets.Server.SSMG_RING_RIGHT_UPDATE();
                p1.CharID = newmaster.CharID;
                p1.Right = ring.Rights[ring.IndexOf(newmaster)].Value;
                if (i.Online)
                    MapClient.FromActorPC(i).netIO.SendPacket(p1);
            }
        }
    }
}
