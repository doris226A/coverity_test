...
namespace SagaMap.Manager
{
    public class PartyManager:Singleton<PartyManager>
    {
        ...
        {
            Party res;
            if (pattern == null || pattern.ID == 0) return null; // 如果 pattern 為空或 ID 為 0，直接返回 null
        ...
        public void PartyDismiss(Party party)
        {
            ...
            foreach (ActorPC i in party.Members.Values)
            {
                try
                {
                    ...
                    if (i.AncientArkID != 0)//ECOKEY 圖書館
                    {
                        if(SagaMap.AncientArks.AncientArkFactory.Instance.AncientArks.ContainsKey(i.AncientArkID)) SagaMap.AncientArks.AncientArkFactory.Instance.AncientArks[i.AncientArkID].Destory(SagaMap.AncientArks.DestroyType.PartyDismiss);
                        i.AncientArkID = 0;
                    }
                    if (!i.Online) continue;
