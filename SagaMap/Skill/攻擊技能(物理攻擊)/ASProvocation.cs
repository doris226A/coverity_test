...
namespace SagaMap.Skill.SkillDefinations.Swordman
{
    ...
    /// 挑釁
    /// </summary>
    public class ASProvocation : ISkill
    {
        ...
        MobType.UNDEAD_BOSS_BSEARCH_SKILL_NOTPTDROPRANGE,MobType.UNDEAD_BOSS_NOTPTDROPRANGE,MobType.MAGIC_CREATURE_BOSS_BSEARCH,MobType.MAGIC_CREATURE_BOSS_BSEARCH_SKILL,MobType.MAGIC_CREATURE_BOSS_BSEARCH_SKILL_NOTPTDROPRANGE,
                ...
                if (!typesToIgnore.Contains(mob.BaseData.mobType))
                {
                    ...
                    if (mobAI != null)
                    {
                        if (sActor.type == ActorType.PARTNER) // 檢查施術者是否是玩家
                        {
                            mobAI.firstAttacker = sActor; // 將施法者設置為 firstAttacker

                            // 增加仇恨值，您可以根據需要進行修改
                            int addedHate = 100; // 增加的仇恨值

                            if (mobAI.Hate.ContainsKey(sActor.ActorID))
                            {
                                mobAI.Hate[sActor.ActorID] += (uint)addedHate;
                            }
                            else
                            {
                                mobAI.Hate.TryAdd(sActor.ActorID, (uint)addedHate);
                            }
                            mobAI.attackStamp = DateTime.Now;
                            // 根據您的需求，您可以添加不同的條件來決定是否強制攻擊施術者
                            // 例如，您可以使用一定的機率讓怪物攻擊施術者
                            int probability = 100; // 40%的機率
                            if (100 <= probability)
                            {
                                mobAI.AttackActor(sActor.ActorID); // 使用 sActor.ActorID 作為攻擊目標的 actorID
                            }
                        }
                    }
                }
                else
                {
                    ...
                    if (mobAI != null)
                    {
                        if (sActor.type == ActorType.PARTNER) // 檢查施術者是否是玩家
                        {
                            int addedHate1 = 0; // 增加的仇恨值
                            if (mobAI.Hate.ContainsKey(sActor.ActorID))
                            {
                                mobAI.Hate[sActor.ActorID] += (uint)addedHate1;
                            }
                            else
                            {
                                mobAI.Hate.TryAdd(sActor.ActorID, (uint)addedHate1);
                            }
                        }
                    }
                }
            ...
            /*  if (dActor.type == ActorType.MOB)
              {
                  ...
                  if (mobAI != null)
                  {
                      if (sActor.type == ActorType.PARTNER) // 檢查施術者是否是玩家
                      {
                          ...
                          // 根據怪物類型進行特殊處理
                          if (MobType == MobType.CC_BOSS)
                          {
                              mobAI.Hate[sActor.ActorID] = 0; // 仇恨值設為 0
                          }
                          ...
                          {
                              mobAI.Hate.Add(sActor.ActorID, (uint)addedHate);
                          }
