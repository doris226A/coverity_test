...
namespace SagaMap.Skill.Additions.Global
{
    public class DefaultBuff : Addition
    {
        ...
        public DefaultBuff(SagaDB.Skill.Skill skill, Actor actor, string name, int lifetime, int period)
        {
            ...
            this.period = period;
            this.EcoKeySkillType = AdditionType.Buff;//ECOKEY 技能類別新增
        }
        //ECOKEY 過圖不消失的類別
        public DefaultBuff(SagaDB.Skill.Skill skill, Actor actor, string name, int lifetime, int period, bool Timeritem)
        {
            if (Timeritem)
            {
                this.Name = name;
                ...
                this.lifeTime = lifetime;
                this.period = period;
                this.EcoKeySkillType = AdditionType.PassiveSkill;
            }
        }
        ...
        public override void AdditionEnd()
        {
            ...
            //ECOKEY 新增寵物數值變化
            if (this.AttachedActor.type == ActorType.PARTNER)
            {
                ActorPartner par = (ActorPartner)this.AttachedActor;
                Partner.StatusFactory.Instance.CalcPartnerStatus(par);
                //加個保險
                if (par.Owner != null)
                {
                    Network.Client.MapClient mc = Network.Client.MapClient.FromActorPC(par.Owner);
                    if (mc != null)
                    {
            ...
            /*  //ECOKEY 新增寵物數值變化
              if (this.AttachedActor.type == ActorType.PARTNER)
              {
                  ActorPartner par = (ActorPartner)this.AttachedActor;
                  Partner.StatusFactory.Instance.CalcPartnerStatus(par);
                  Network.Client.MapClient.FromActorPC(par.Owner).SendPetDetailInfo();
              }*/
        }
