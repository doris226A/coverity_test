...
namespace SagaMap.Skill.SkillDefinations.Cabalist
{
    ...
    /// 睡眠霧氣（スリープクラウド）
    /// </summary>
    public class LFRSleepCloud : ISkill
    {
                    ...
                    if (count < times)
                    {
                        ...
                        foreach (Actor act in affected)
                        {
                            if (SkillHandler.Instance.CheckValidAttackTarget(sActor, act))
                            {
                                ...
                                {
                                    if (!act.Status.Additions.ContainsKey("Sleep"))
                                    {
                                        // 施加睡眠状态
                                ...
                                {
                                    if (!act.Status.Additions.ContainsKey("Poison"))
                                    {
                                        // 施加睡眠状态
                                        Poison s = new Poison(skill.skill, act, 5000);
                                        SkillHandler.ApplyAddition(act, s);
                                    }
