...
namespace SagaMap.Skill.SkillDefinations.Sorcerer
{
    ...
    /// 神聖光界（ディバインバリア）
    /// </summary>
    public class LFRDevineBarrier : ISkill
    {
            ...
            foreach (Actor act in affected)
            {
                if (act.type == ActorType.PC || act.type == ActorType.PARTNER || act.type == ActorType.PET)
                {
                    if (act.type == ActorType.PC)
                    {
                        ...
                        if (pc.PossessionTarget > 0 || SkillHandler.Instance.CheckValidAttackTarget(sActor, act))
                        {
                            continue;
                        }
                    }
                    SkillHandler.RemoveAddition(act, "MagicShield");
                    SkillHandler.RemoveAddition(act, "EnergyShield");
                    SkillHandler.RemoveAddition(act, "MagicBarrier");
                    SkillHandler.RemoveAddition(act, "EnergyBarrier");
        ...
        {
            if (actor.type == ActorType.PC || actor.type == ActorType.PARTNER || actor.type == ActorType.PET)
            {
