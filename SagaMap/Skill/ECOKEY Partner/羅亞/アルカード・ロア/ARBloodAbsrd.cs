...
namespace SagaMap.Skill.SkillDefinations.DarkStalker
{
    ...
    /// 血液吸收（ブラッドアブソーブ）
    /// </summary>
    public class ARBloodAbsrd : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            {
                if (SkillHandler.Instance.CheckValidAttackTarget(sActor, i) && i.HP > 0)
                {
                    SkillHandler.Instance.ShowVessel(i, 0, 0, (int)-(dmgheal * 0.3f));
                    i.SP -= (uint)-(dmgheal * 0.3f);
                    if (i.SP < 0)
                        i.SP = 0;
