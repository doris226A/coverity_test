    ...
    public partial class SkillHandler : Singleton<SkillHandler>
    {
        ...
        public int checkraceskill(Actor sActor, Actor dActor, int damage)
        {
            ...
            else if (sActor.type == ActorType.PC)
            {
                ...
                if (pc.Skills2_1.ContainsKey(500) || pc.DualJobSkill.Exists(x => x.ID == 500))//精灵/木偶种族伤害提升 増加率	3％	6％	9％	12％	15％
                {
                    var duallv = 0;
        ...
        {
            return checkirisbuff(sActor, dActor, arg, type, damage, Elements.Neutral);
        }
        ...
        public int checkirisbuff(Actor sActor, Actor dActor, SkillArg arg, byte type, int damage, Elements element)
        {
            ...
            else if (type == 1)
            {
                if (sActor.type == ActorType.PC)
                {
                    ...
                    //iris卡种族增伤部分
                    if (dActor.Race == Race.BIRD && pc.Status.human_dmg_up_iris > 100)
                    {
                        damage = (int)(damage * (float)(pc.Status.human_dmg_up_iris / 100.0f));
                    }
                    else if (dActor.Race == Race.HUMAN && pc.Status.bird_dmg_up_iris > 100)
                    {
                        damage = (int)(damage * (float)(pc.Status.bird_dmg_up_iris / 100.0f));
                    }
                    else if (dActor.Race == Race.ANIMAL && pc.Status.animal_dmg_up_iris > 100)
                ...
                if (dActor.type == ActorType.PC)
                {
                    ...
                    if (dActor.Status.magic_rate_iris < 100 && damage > 0)//iris卡片提供的魔法伤害减少
                    {
                        damage = (int)(damage * (float)(dActor.Status.magic_rate_iris / 100.0f));
                    }
                    //iris卡种族减伤部分
                    if (sActor.Race == Race.HUMAN && pc.Status.human_dmg_down_iris < 100)
                    {
        ...
        public int CalcDamage(bool IsPhyDamage, Actor sActor, Actor dActor, SkillArg arg, DefType defType, Elements element, int eleValue, float ATKBonus, out AttackResult res, int scribonus, int cridamagebonus, float ignore = 0)
        {
            try
            {
                ...
                if (IsPhyDamage)
                {
                    ...
                    else if (arg.skill.ID == 2546)
                    {
                        switch (arg.type)
                        {
                            case ATTACK_TYPE.BLOW:
                                mindamage = sActor.Status.max_atk1;
                                maxdamage = sActor.Status.max_atk1;
                                break;
                            case ATTACK_TYPE.SLASH:
                                mindamage = sActor.Status.max_atk2;
                                maxdamage = sActor.Status.max_atk2;
                                break;
                            case ATTACK_TYPE.STAB:
                                mindamage = sActor.Status.max_atk3;
                    ...
                    if (element != Elements.Neutral)
                        eleBonus = CalcElementBonus(sActor, dActor, element, 0, false);
                    ...
                    if (sActor.Status.Additions.ContainsKey("FrameHart") && !myrobot)//火心
                    {
                    ...
                    if (res == AttackResult.Critical)
                    {
                        var critdmgrate = Math.Min(CalcCritBonus(sActor, dActor, scribonus), 2.0f);
                    ...
                    damage = checkirisbuff(sActor, dActor, arg, 0, damage, element);
                ...
                else
                {
                    if (arg != null)
                    {
                        if (arg.skill.ID == 2546)
                        {
                            mindamage = sActor.Status.max_matk;
                    ...
                    if (element != Elements.Neutral)
                    {
                        float eleBonuss = CalcElementBonus(sActor, dActor, element, 1, ((ATKBonus < 0) && !((dActor.Status.undead == true) && (element == Elements.Holy))), arg.skill.ID);
                        if (sActor.Status.Contract_Lv != 0)//CAJOB40
                        {
                            ...
                            if (tmpele == element)
                                eleBonuss += 0.5f;
                            else if(element != Elements.Neutral || element != Elements.Dark || element != Elements.Holy)
                                eleBonuss -= 0.65f;
                    ...
                    if (!myrobot)
                    {
                        damage = checkirisbuff(sActor, dActor, arg, 1, damage, element);
                    }
                    checkdebuff(sActor, dActor, arg, 1);
                }
                damage = checkraceskill(sActor, dActor, damage);
                ...
                if (map.CheckActorSkill(dActor.X, dActor.Y, 200, 2612) && dActor.type != ActorType.MOB)//阿瓦隆状态下,减伤50%
        ...
        public bool CheckSkillCanCastForWeapon(ActorPC pc, SkillArg arg)
        {
            ...
            else if (arg.skill.BaseData.equipFlag.Test(EquipFlags.NONE) || arg.skill.BaseData.equipFlag.Test(EquipFlags.HAND))
                return true;
        ...
        /// <summary>
        /// 即死判定(必定成功,概率在外部填写,以及对BOSS绝对无效)
        /// </summary>
        /// <param name="actor">死亡对象</param>
        /// <param name="arg">技能参数</param>
        public void OnceDie(Actor actor, SkillArg arg)
        {
            if (actor.type == ActorType.MOB)
            {
                ActorMob dActorMob = (ActorMob)actor;
            ...
            arg.affectedActors.Add(actor);
            arg.Init();
            arg.flag[0] = SagaLib.AttackFlag.DIE;
        }
        /// <summary>
        /// 返回范围内可被攻击的对象
        /// </summary>
