    ...
    public partial class SkillHandler
    {
        ...
        public AttackResult CalcAttackResult(Actor sActor, Actor dActor, bool ranged, int shitbonus, int scribonus)
        {
            ...
            if (dActor.type == ActorType.PC)
            {
                ...
                //ECOKEY 厄運修復（整段取代）
                if (pc.Skills2.ContainsKey(973) || pc.Skills2_2.ContainsKey(973) || pc.SkillsReserve.ContainsKey(973) || pc.DualJobSkill.Exists(x => x.ID == 973))//厄运，一定概率强制回避物理技能  //2-2
                {
                    //这里取副职的等级
                    var duallv = 0;
                    ...
                    //这里取主职的等级
                    var mainlv = 0;
                    if (pc.Skills2.ContainsKey(973)) mainlv = pc.Skills2[973].Level;
                    if (pc.Skills2_2.ContainsKey(973)) mainlv = pc.Skills2_2[973].Level;
                    if (pc.SkillsReserve.ContainsKey(973)) mainlv = pc.SkillsReserve[973].Level;
            ...
            if (dActor.Status.Additions.ContainsKey("GSParry"))//守護者之盾
            {
                if (SagaLib.Global.Random.Next(0, 99) < 10)
                    return AttackResult.GSParry;
            }
            ...
            // 確保 dActor 的狀態加成不為空
            if (dActor.Status.Additions != null && dActor.Status.Additions.ContainsKey("FortressCircleSEQ"))
            {
            ...
            //ECOKEY 迴避率被動系列
            dAvoid = checkraceavoid(sActor, dActor, dAvoid);
            ...
            float leveldiff = sActor.Level - dActor.Level;
        ...
        public AttackResult CalcMagicAttackResult(Actor sActor, Actor dActor)
        {
            ...
            if (dActor.type == ActorType.PC)
            {
                ActorPC pc = (ActorPC)dActor;
                //ECOKEY 強運修復（整段取代）
                if (pc.Skills2_2.ContainsKey(960) || pc.Skills2.ContainsKey(960) || pc.SkillsReserve.ContainsKey(960) || pc.DualJobSkill.Exists(x => x.ID == 960))//强运，一定概率强制回避魔法技能
                {
                    //这里取副职的等级
                    var duallv = 0;
                    ...
                    //这里取主职的等级
                    var mainlv = 0;
                    if (pc.Skills2_2.ContainsKey(960)) mainlv = pc.Skills2_2[960].Level;
                    if (pc.Skills2.ContainsKey(960)) mainlv = pc.Skills2[960].Level;
                    if (pc.SkillsReserve.ContainsKey(960)) mainlv = pc.SkillsReserve[960].Level;
