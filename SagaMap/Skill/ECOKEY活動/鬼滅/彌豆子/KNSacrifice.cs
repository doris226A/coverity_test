...
namespace SagaMap.Skill.SkillDefinations.Swordman
{
    ...
    /// 
    /// </summary>
    public class KNSacrifice : ISkill
    {
        DefaultBuff oldSacrifice = null;
        public int TryCast(ActorPC sActor, Actor dActor, SkillArg args)
        {
            if (dActor.HP < (uint)(dActor.MaxHP * 0.08))
                return -12;
            else
            {
                if (dActor.Status.Additions.ContainsKey("ForceMaster"))
                {
                    return -12;
                }
                return 0;
        ...
        {
            float hpdmg = (float)(dActor.MaxHP * 0.2);
            int lifetime = 24000 + 12000;
            if (dActor.Status.Additions.ContainsKey("Sacrifice"))
                oldSacrifice = dActor.Status.Additions["Sacrifice"] as DefaultBuff;
            else
                oldSacrifice = null;
            DefaultBuff skill = new DefaultBuff(args.skill, dActor, "Sacrifice", lifetime);
            skill.OnAdditionStart += this.StartEventHandler;
            skill.OnAdditionEnd += this.EndEventHandler;
            SkillHandler.ApplyAddition(dActor, skill);
            dActor.HP -= (uint)hpdmg;
            SkillHandler.Instance.ShowVessel(dActor, (int)hpdmg);
        }
        void StartEventHandler(Actor actor, DefaultBuff skill)
        {
            if (actor.Status.Additions.ContainsKey("ForceMaster"))
            {
                ...
                actor.Status.Additions.TryRemove("ForceMaster", out _);
            }
