    ...
    public partial class SkillHandler : Singleton<SkillHandler>
    {
        ...
        public Actor GetdActor(Actor sActor, SkillArg arg)
        {
            ...
            if (target == null || target.HP <= 0)
            {
                ...
                {
                    MapClient.FromActorPC((ActorPC)sActor).SendSystemMessage("沒有鎖定的目標，輸入/autolock開啟與關閉自動鎖定模式。");
                    return null;
                }
                else
                {
                    // List<Actor> actors = map.GetActorsArea(sActor, (short)((arg.skill.Range + 2) * 100), false);
                    List<Actor> actors = map.GetActorsArea(sActor, (short)((arg.skill.Range + 2) * 65), false);
                    ...
                    if (Targets.Count < 1)
                    {
                        MapClient.FromActorPC((ActorPC)sActor).SendSystemMessage("技能範圍內沒有目標可以鎖定。");
                        return null;
                    }
                    else
                        target = Targets[0];
            ...
            if (arg.skill.Range + 2 < Math.Max(Math.Abs​​(sActor.X - target.X) / 65, Math.Abs​​(sActor.Y - target.Y) / 65))
            {
                MapClient.FromActorPC((ActorPC)sActor).SendSystemMessage("【你目前鎖定的目標】超出了技能的極限範圍。");
                return null;
            }

            if (target.HP <= 0)
            {
                MapClient.FromActorPC((ActorPC)sActor).SendSystemMessage("鎖定的目標已死亡或已不存在。");
                return null;
            }
        ...
        /// <summary>
        /// 检查一部分种族针对性被动技能对伤害的影响
        /// </summary>
        /// <param name="sActor">攻击者</param>
        /// <param name="dActor">目标</param>
        public int checkraceskill(Actor sActor, Actor dActor, int damage)
        {
            if (sActor.type != ActorType.PC) return damage;
            else if (sActor.type == ActorType.PC)
            {
                ActorPC pc = (ActorPC)sActor;
                if (pc.Skills2_2.ContainsKey(400) || pc.DualJobSkill.Exists(x => x.ID == 400))//人类种族伤害提升//ダメージ増加率(%)	3	6	9	12	15
                {
                    var duallv = 0;
                    if (pc.DualJobSkill.Exists(x => x.ID == 400))
                        duallv = pc.DualJobSkill.FirstOrDefault(x => x.ID == 400).Level;

                    var mainlv = 0;
                    if (pc.Skills2_2.ContainsKey(400))
                        mainlv = pc.Skills2_2[400].Level;
                    if (dActor.Race == Race.HUMAN || dActor.type == ActorType.PC)//ECOKEY 人類系新增玩家
                    {
                        damage = (int)(damage * (1.0f + 0.03f * Math.Max(duallv, mainlv)));
                    }

                }
                //已确认动物系伤害提升玩家不存在该技能
                if (pc.Skills2_1.ContainsKey(406) || pc.DualJobSkill.Exists(x => x.ID == 406))//植物种族伤害提升//ダメージ倍率	12%	14%	16%	18%	20%
                {
                    var duallv = 0;
                    if (pc.DualJobSkill.Exists(x => x.ID == 406))
                        duallv = pc.DualJobSkill.FirstOrDefault(x => x.ID == 406).Level;

                    var mainlv = 0;
                    if (pc.Skills2_1.ContainsKey(406))
                        mainlv = pc.Skills2_1[406].Level;
                    if (dActor.Race == Race.PLANT)
                    {
                        damage = (int)(damage * (1.0f + 0.1f + 0.02f * Math.Max(duallv, mainlv)));
                    }

                }
                if (pc.Skills2_1.ContainsKey(409) || pc.DualJobSkill.Exists(x => x.ID == 409))//岩石种族伤害提升 wiki未写,采纳以上同为2-1的植物系系数
                {
                    var duallv = 0;
                    if (pc.DualJobSkill.Exists(x => x.ID == 409))
                        duallv = pc.DualJobSkill.FirstOrDefault(x => x.ID == 409).Level;

                    var mainlv = 0;
                    if (pc.Skills2_1.ContainsKey(409))
                        mainlv = pc.Skills2_1[409].Level;
                    if (dActor.Race == Race.ROCK)
                    {
                        damage = (int)(damage * (1.0f + 0.1f + 0.02f * Math.Max(duallv, mainlv)));
                    }
                ...
                //已确认昆虫系伤害提升玩家不存在该技能
                //已确认水中生物系伤害提升玩家不存在该技能
                //ECOKEY 精靈種族傷害提升修復（暗黑騎士此技能在2-2）
                if (pc.Skills2_1.ContainsKey(500) || pc.Skills2_2.ContainsKey(500) || pc.DualJobSkill.Exists(x => x.ID == 500))//精灵/木偶种族伤害提升 増加率	3％	6％	9％	12％	15％
                {
                    var duallv = 0;
                    if (pc.DualJobSkill.Exists(x => x.ID == 500))
                        duallv = pc.DualJobSkill.FirstOrDefault(x => x.ID == 500).Level;

                    var mainlv = 0;
                    if (pc.Skills2_1.ContainsKey(500))
                        mainlv = pc.Skills2_1[500].Level;
                    if (pc.Skills2_2.ContainsKey(500))
                        mainlv = pc.Skills2_2[500].Level;
                    if (dActor.Race == Race.ELEMENT)
                    {
                        damage = (int)(damage * (1.0f + 0.03f * Math.Max(duallv, mainlv)));
                    }

                }
                //已确认魔法生物系伤害提升玩家不存在该技能
                //已确认机械生物系伤害提升玩家不存在该技能
                if (pc.Skills2_1.ContainsKey(509) || pc.DualJobSkill.Exists(x => x.ID == 509))//不死生物种族伤害提升 ダメージ上昇率(%)	+3%	+6%	+9%	+12%	+15%
                {
                    var duallv = 0;
                    if (pc.DualJobSkill.Exists(x => x.ID == 509))
                        duallv = pc.DualJobSkill.FirstOrDefault(x => x.ID == 509).Level;

                    var mainlv = 0;
                    if (pc.Skills2_1.ContainsKey(509))
                        mainlv = pc.Skills2_1[509].Level;
                    if (dActor.Race == Race.UNDEAD)
                    {
                        damage = (int)(damage * (1.0f + 0.03f * Math.Max(duallv, mainlv)));
                    }

                }
                //已确认生物系(?)(SkillID:512)伤害提升玩家不存在该技能
                if(pc.TInt["AA_Race_Attack"] != 0)//ECOKEY 圖書館設計，種族傷害差異
                {
                    damage = (int)(damage * (1.0f + 50.0f / pc.Level));
                }
            }
            return damage;
        }
        ...
        //ECOKEY 迴避率被動系列
        /// <summary>
        /// 檢查一部分種族針對性被動技能對迴避率的影響
        /// </summary>
        /// <param name="sActor">攻擊者</param>
        /// <param name="dActor">目標</param>
        public int checkraceavoid(Actor sActor, Actor dActor, int avoid)
        {
            if (dActor.type != ActorType.PC) return avoid;
            else if (dActor.type == ActorType.PC)
            {
                ActorPC pc = (ActorPC)dActor;
                if (pc.Skills2_2.ContainsKey(402) || pc.DualJobSkill.Exists(x => x.ID == 402))//人类种族伤害提升//ダメージ増加率(%)	3	6	9	12	15
                {
                    var duallv = 0;
                    if (pc.DualJobSkill.Exists(x => x.ID == 402))
                        duallv = pc.DualJobSkill.FirstOrDefault(x => x.ID == 402).Level;

                    var mainlv = 0;
                    if (pc.Skills2_2.ContainsKey(402))
                        mainlv = pc.Skills2_2[402].Level;
                    if (sActor.Race == Race.HUMAN || sActor.type == ActorType.PC)
                    {
                        avoid = (int)(avoid * (1.0f + 0.03f * Math.Max(duallv, mainlv)));
                    }
        ...
        public int checkirisbuff(Actor sActor, Actor dActor, SkillArg arg, byte type, int damage, Elements element)
        {
        ...
        public int CalcDamage(bool IsPhyDamage, Actor sActor, Actor dActor, SkillArg arg, DefType defType, Elements element, int eleValue, float ATKBonus, out AttackResult res, int scribonus, int cridamagebonus, float ignore = 0)
        {
            try
            {
                ...
                if (res == AttackResult.Critical)
                {
                    var critdmgrate = Math.Min(CalcCritBonus(sActor, dActor, 0), 2.0f);
                    damage = (int)(((float)damage) * critdmgrate);
                    if (sActor.Status.Additions.ContainsKey("CriDamUp"))
                    {
                        float rate = (float)((float)(sActor.Status.Additions["CriDamUp"] as DefaultPassiveSkill).Variable["CriDamUp"] / 100.0f + 1.0f);
                        damage = (int)((float)damage * rate);
                ...
                if (IsPhyDamage)
                {
                    ...
                    if (sActor.type == ActorType.PC)
                    {
                        ...
                        if (pc.Skills2_2.ContainsKey(315) || pc.DualJobSkill.Exists(x => x.ID == 315))//ECOKEY SO2-2落井下石
                        {
                            var duallv = 0;
        ...
        public void CastPassiveSkills(ActorPC pc, bool ReCalcState = true)
        {
            ...
            if(pc.Race == PC_RACE.DEM && pc.Status.min_atk_bs == 0) return;//ECOKEY 強制解決DEM會因裝備技能卡登入的問題
        ...
        public static void ApplyAddition(Actor actor, Addition addition)
        {
            ...
            if (actor.type == ActorType.PC)
            {
                ...
                if (actor.Buff.EnchantmentBlock)//ECOKEY 驅散狀態禁止上BUFF
                {
            ...
            if (actor.Status.Additions.ContainsKey(addition.Name))
            {
                ...
                actor.Status.Additions.TryRemove(addition.Name, out _);
                actor.Status.Additions.TryAdd(addition.Name, addition);
        ...
        {
            //ECOKEY 旋風刺找不到原因，強制修復
            if (arg.skill.ID == 2394)
                return true;
            if (arg.skill.BaseData.equipFlag.Value == 0)
                return true;
            if (pc.Race == PC_RACE.DEM && pc.Form == DEM_FORM.MACHINA_FORM)//ECOKEY DEM技能裝備判斷0124再改
            {
                Dictionary<EnumEquipSlot, Item> equips = pc.Inventory.Parts;
                if (equips.ContainsKey(EnumEquipSlot.RIGHT_HAND))
                {
                    List<ItemType> i = new List<ItemType>();
                    switch (equips[EnumEquipSlot.RIGHT_HAND].BaseData.itemType)
                    {
                        case ItemType.PARTS_BLOW:
                        case ItemType.SETPARTS_BLOW:
                            i.Add(ItemType.PARTS_BLOW);
                            //i.Add(ItemType.SETPARTS_BLOW);
                            i.Add(ItemType.STRINGS);
                            i.Add(ItemType.STAFF);
                            i.Add(ItemType.AXE);
                            i.Add(ItemType.HAMMER);
                            i.Add(ItemType.HANDBAG);
                            break;
                        case ItemType.PARTS_SLASH:
                        case ItemType.SETPARTS_SLASH:
                            i.Add(ItemType.PARTS_SLASH);
                            //i.Add(ItemType.SETPARTS_SLASH);
                            i.Add(ItemType.SWORD);
                            i.Add(ItemType.CARD);
                            i.Add(ItemType.SHORT_SWORD);
                            break;
                        case ItemType.PARTS_STAB:
                        case ItemType.SETPARTS_STAB:
                            i.Add(ItemType.PARTS_STAB);
                            //i.Add(ItemType.SETPARTS_STAB);
                            i.Add(ItemType.CLAW);
                            i.Add(ItemType.SPEAR);
                            i.Add(ItemType.RAPIER);
                            i.Add(ItemType.THROW);
                            break;
                        case ItemType.PARTS_LONGRANGE:
                        case ItemType.SETPARTS_LONGRANGE:
                            i.Add(ItemType.PARTS_LONGRANGE);
                            //i.Add(ItemType.SETPARTS_LONGRANGE);
                            i.Add(ItemType.RIFLE);
                            i.Add(ItemType.BOW);
                            i.Add(ItemType.GUN);
                            i.Add(ItemType.DUALGUN);
                            i.Add(ItemType.ARROW);
                            break;
                    }
                    foreach (ItemType k in i)
                    {
                        if (arg.skill.BaseData.equipFlag.Test((EquipFlags)Enum.Parse(typeof(EquipFlags), k.ToString())))
                            return true;
                    }
                }
            ...
            else if (pc.Inventory.Equipments.ContainsKey(EnumEquipSlot.LEFT_HAND))
            {
                ...
                return true;
            else if (arg.skill.BaseData.equipFlag.Test(EquipFlags.NONE))//ECOKEY 新增空手判斷
                return true;

            if (arg.skill.BaseData.equipFlag.Value == 2013265920)//ECOKEY 新增雙手武器判斷
            {
                ...
                {
                    switch (pc.Inventory.Equipments[EnumEquipSlot.RIGHT_HAND].BaseData.itemType)
                    {
        ...
        public bool CheckValidAttackTarget(Actor sActor, Actor dActor)
        {
                    ...
                    case ActorType.PC:
                        {
                            ...
                            //ECOKEY 英雄狀態判斷
                            if (pc.Mode == PlayerMode.KNIGHT_EAST_HERO || pc.Mode == PlayerMode.KNIGHT_NORTH_HERO || pc.Mode == PlayerMode.KNIGHT_SOUTH_HERO || pc.Mode == PlayerMode.KNIGHT_WEST_HERO)
                            {
                                string mode = pc.Mode.ToString();
                                if (mode.Contains(target.Mode.ToString()))
                                    return false;
                                else
                                    return true;
                            }
                            else if (target.Mode == PlayerMode.KNIGHT_EAST_HERO || target.Mode == PlayerMode.KNIGHT_NORTH_HERO || target.Mode == PlayerMode.KNIGHT_SOUTH_HERO || target.Mode == PlayerMode.KNIGHT_WEST_HERO)
                            {
                                string mode = target.Mode.ToString();
                                if (mode.Contains(pc.Mode.ToString()))
                                    return false;
                                else
                                    return true;
                            }
            ...
            else if (sActor.type == ActorType.MOB)
            {
                ...
                else
                {
                    switch (dActor.type)
                    {
                        case ActorType.MOB://ECOKEY 禁止打象徵
                            eh = (SagaMap.ActorEventHandlers.MobEventHandler)dActor.e;
                            if (eh.AI.Mode.Symbol)
                                return false;
                            else
                                return true;
                        case ActorType.PARTNER:
                            return false;  //ECOKEY 禁止召喚物打寵物
                        //ECOKEY 召喚物打玩家
                        case ActorType.PC:
                            {
                                ActorPC pc = (ActorPC)eh.AI.Master;
                                ActorPC target = (ActorPC)dActor;
                                if (pc == target)
                                    return false;
                                if ((pc.Mode == PlayerMode.COLISEUM_MODE && target.Mode == PlayerMode.COLISEUM_MODE) ||
            ...
            else if (sActor.type == ActorType.PARTNER)
            {
                ...
                {
                    case ActorType.MOB://ECOKEY 禁止打象徵
                        ActorEventHandlers.MobEventHandler eh = (SagaMap.ActorEventHandlers.MobEventHandler)dActor.e;
                        if (eh.AI.Mode.Symbol)
                            return false;
                        else
                            return true;
            ...
            else if (sActor.type == ActorType.SHADOW)//ECOKEY 新增SHADOW判斷
            {
                switch (dActor.type)
                {
                    case ActorType.MOB://ECOKEY 禁止打象徵
                    ...
                    case ActorType.PC:
                        {
                            ActorShadow pet = (ActorShadow)sActor;
                            if (pet.Owner != null && pet.Owner.ActorID != dActor.ActorID)
                            {
                                ActorPC pc = (ActorPC)pet.Owner;
                                ActorPC target = (ActorPC)dActor;
                                if (pc.Mode == PlayerMode.KNIGHT_EAST_HERO || pc.Mode == PlayerMode.KNIGHT_NORTH_HERO || pc.Mode == PlayerMode.KNIGHT_SOUTH_HERO || pc.Mode == PlayerMode.KNIGHT_WEST_HERO)
                                {
                                    string mode = pc.Mode.ToString();
