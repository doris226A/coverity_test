    ...
    public class Gospel : ISkill
    {
        ...
        bool CheckPossible(Actor sActor)
        {
            if (sActor.type == ActorType.PC)
            {
                ...
                if (Skill.SkillHandler.Instance.isEquipmentRight(pc, SagaDB.Item.ItemType.STRINGS) || pc.Inventory.GetContainer(SagaDB.Item.ContainerType.RIGHT_HAND2).Count > 0)
                    return true;
                else if (Skill.SkillHandler.Instance.isEquipmentRight(pc, SagaDB.Item.ItemType.STAFF) || pc.Inventory.GetContainer(SagaDB.Item.ContainerType.RIGHT_HAND2).Count > 0)
                    return true;
                else
                    return false;
            }
        ...
        private class Activator : MultiRunTask
        {
                    ...
                    if (count < countMax)
                    {
                        ...
                        foreach (Actor i in actors)
                        {
                            if (i.type == ActorType.PC)
                            {
                                ...
                                if (!SkillHandler.Instance.CheckValidAttackTarget(caster, pc) && pc.Online && !pc.Buff.Dead && pc.PossessionTarget == 0)
                                {
                                    affected.Add(pc);
                            ...
                            else if (i.type == ActorType.PARTNER || i.type == ActorType.PET)
                            {
                                if (!SkillHandler.Instance.CheckValidAttackTarget(caster, i) && !i.Buff.Dead)
                                {
                                    affected.Add(i);
                                    if (!i.Status.Additions.ContainsKey("GospelBonus"))
                                    {
                                        DefaultBuff gospelBonus = new DefaultBuff(skill.skill, i, "GospelBonus", lifetime);
                                        gospelBonus.OnAdditionStart += gospelBonus_OnAdditionStart;
                                        gospelBonus.OnAdditionEnd += gospelBonus_OnAdditionEnd;
                                        SkillHandler.ApplyAddition(i, gospelBonus);
                                    }
                                }
                            }
            ...
            void gospelBonus_OnAdditionStart(Actor actor, DefaultBuff skill)
            {
                ...
                uint hpbonus = (uint)((float)(actor.MaxHP - (actor.Status.hp_item + actor.Status.hp_mario + actor.Status.hp_skill + actor.Status.hp_iris + actor.Status.hp_another)) * (((float)(10 + 5 * level) / 100.0f)));
                uint mpbonus = (uint)((float)(actor.MaxMP - (actor.Status.mp_item + actor.Status.mp_mario + actor.Status.mp_skill + actor.Status.mp_iris + actor.Status.mp_another)) * (((float)(10 + 5 * level) / 100.0f)));
                uint spbonus = (uint)((float)(actor.MaxSP - (actor.Status.sp_item + actor.Status.sp_mario + actor.Status.sp_skill + actor.Status.sp_iris + actor.Status.sp_another)) * (((float)(10 + 5 * level) / 100.0f)));
