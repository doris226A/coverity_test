...
namespace SagaMap.Skill.SkillDefinations.ForceMaster
{
    ...
    public class ForceShield : ISkill
    {
        ...
        {
            if (sActor != dActor)// && dActor.type == ActorType.PC && dActor.HP < dActor.MaxHP * 0.4)
            {
                ActorPC realDActor = SkillHandler.Instance.GetPossesionedActor((ActorPC)dActor);
                int lifetime = 150000 + 30000 * level;
                DefaultBuff skill = new DefaultBuff(args.skill, realDActor, "ForceShield", lifetime);
                skill.OnAdditionStart += this.StartEventHandler;
                skill.OnAdditionEnd += this.EndEventHandler;
                SkillHandler.ApplyAddition(dActor, skill);
                SkillHandler.Instance.ShowEffectOnActor(realDActor, 4019);
            }
        }
        ...
        void StartEventHandler(Actor actor, DefaultBuff skill)
        {
            ...
            SagaMap.Network.Client.MapClient.FromActorPC(pc).SendSystemMessage("原力護盾被觸發！");
        }
        ...
        void EndEventHandler(Actor actor, DefaultBuff skill)
        {
            ...
            ActorPC pc = (ActorPC)actor;
            SagaMap.Network.Client.MapClient.FromActorPC(pc).SendSystemMessage("原力護盾消失了！");
        }
