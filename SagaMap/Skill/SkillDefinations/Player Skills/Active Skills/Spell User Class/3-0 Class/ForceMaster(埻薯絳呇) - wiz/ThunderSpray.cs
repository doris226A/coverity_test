        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            foreach (Actor i in actors)
            {
                if (SkillHandler.Instance.CheckValidAttackTarget(sActor, i))
                {
                    ...
                    {
                        int rate = 10;
                        if (!i.Status.Additions.ContainsKey("Frustrate"))//不处于挫败DEBUFF下
                        {
                            ActorPC pc = (ActorPC)sActor;
                            if (pc.Skills2_1.ContainsKey(3255) || pc.DualJobSkill.Exists(x => x.ID == 3255))
                            {
                                var duallv = 0;
                                if (pc.DualJobSkill.Exists(x => x.ID == 3255))
                                    duallv = pc.DualJobSkill.FirstOrDefault(x => x.ID == 3255).Level;

                                var mainlv = 0;
                                if (pc.Skills2_1.ContainsKey(3255))
                                    mainlv = pc.Skills2_1[3255].Level;

                                rate += 5 * Math.Max(duallv, mainlv);
                            }
                            if (pc.Skills2_1.ContainsKey(3256) || pc.DualJobSkill.Exists(x => x.ID == 3256))
                            {
                                var duallv = 0;
                                if (pc.DualJobSkill.Exists(x => x.ID == 3256))
                                    duallv = pc.DualJobSkill.FirstOrDefault(x => x.ID == 3256).Level;

                                var mainlv = 0;
                                if (pc.Skills2_1.ContainsKey(3256))
                                    mainlv = pc.Skills2_1[3256].Level;
