...
namespace SagaMap.Skill.SkillDefinations.Shaman
{
    class FireWall : ISkill
    {
        ...
        private class Activator : MultiRunTask
        {
            ...
            private bool CheckCollision(Actor monster, Actor obstacle)
            {
                // 检查怪物和障碍物的位置关系
                float distanceX = Math.Abs(monster.X - obstacle.X);
                float distanceY = Math.Abs(monster.Y - obstacle.Y);

                // 如果怪物和障碍物的距离小于一定范围，认为发生了碰撞
                float collisionDistanceThreshold = 10.0f; // 调整这个值以适应你的游戏
                if (distanceX < collisionDistanceThreshold && distanceY < collisionDistanceThreshold)
                {
                    return true; // 碰撞发生
                }
            ...
            public override void CallBack()
            {
                ...
                try
                {
                    if (DateTime.Now <= timeMax)  // 持续检测碰撞直到技能的持续时间结束
                    {
                        foreach (ActorSkill i in actorSkill)
                        {
                            //affectedCheck.Clear();
                                ...
                                foreach (Actor j in affected)
                                {
                                    if (!affectedCheck.Contains(j))
                                    {
                                        if (j.type == ActorType.MOB)
                                        {
                                            ...
                                            // 使用 SmoothMove 进行平滑移动
                                        //    short[] destination = new short[] { (short)(j.X + 50), j.Y };
                                         //   j.SmoothMove(destination, 10);  // 10 是移动的帧数，可以根据需要调整
                                               ...
                                               if (j.LastX != 0 && j.LastY != 0)
                                               {
                                                   ...
                                                   pos[0] = j.LastX;
                                                   pos[1] = j.LastY;
                                                   map.MoveActor(Map.MOVE_TYPE.START, j, pos, 500, 0);
