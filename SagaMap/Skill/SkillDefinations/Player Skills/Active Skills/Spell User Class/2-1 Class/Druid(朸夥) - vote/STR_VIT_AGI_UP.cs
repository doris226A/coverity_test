...
namespace SagaMap.Skill.SkillDefinations.Druid
{
    ...
    public class STR_VIT_AGI_UP : ISkill
    {
        ...
        void StartEventHandler(Actor actor, DefaultBuff skill)
        {
            ...
            {
                short minatk = (short)Math.Floor(STR[level - 1] + Math.Pow(Math.Floor((double)(STR[level - 1] / 9)), 2));
                short maxatk = (short)(STR[level - 1] + Math.Pow(Math.Floor((double)((float)(STR[level - 1] + 14) / 5.0f)), 2));
                ...
                short def = (short)((float)(VIT[level - 1] / 3.0f) + (float)(VIT[level - 1] / 4.5f));
                short mdef = (short)((float)(VIT[level - 1] / 3.0f) + (float)(VIT[level - 1] / 4.0f));
                actor.Status.def_skill += def;
                actor.Status.mdef_skill += mdef;
                short hp = (short)(actor.MaxHP * (float)(VIT[level - 1] / 130.0f));
                short sp = (short)(actor.MaxSP * (float)(VIT[level - 1] / 1000.0f * 5.0f));
                actor.Status.hp_skill += hp;
                actor.Status.sp_skill += sp;

                short avoid_melee = (short)(AGI[level - 1] + Math.Pow(Math.Floor((float)(AGI[level - 1] + 18) / 9.0f), 2) + Math.Floor((float)actor.Level / 3.0f) - 1);
                short avoid_ranged = (short)(Math.Floor(AGI[level - 1] + Math.Floor((float)actor.Level / 3.0f) + 3));
                actor.Status.avoid_melee_skill += avoid_melee;
                actor.Status.avoid_ranged_skill += avoid_ranged;

                short aspd = (short)(AGI[level - 1] * 3 + Math.Floor(Math.Pow((short)((float)(AGI[level - 1] + 63) / 9.0f), 2)));
