...
namespace SagaMap.Skill.SkillDefinations.Druid
{
    ...
    public class SealMagic : ISkill
    {
        ...
        {
            // 檢查攻擊是否是魔法攻擊
            /*if (args.skill != null && args.skill.Magical)
            {
            ...
            if (SagaLib.Global.Random.Next(0, 99) < rate)
            {
                ...
                if (dActor.Tasks.ContainsKey("SkillCast"))
                {
                    ...
                    int lifetime = 3000 + 1000 * level;
                    DefaultBuff skill = new DefaultBuff(args.skill, dActor, "SealMagic", lifetime);
        ...
        {
            actor.Buff.MagicSealed = true;
            Manager.MapManager.Instance.GetMap(actor.MapID).SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.BUFF_CHANGE, null, actor, true);
        }
        void EndEventHandler(Actor actor, DefaultBuff skill)
        {
            actor.Buff.MagicSealed = false;
            Manager.MapManager.Instance.GetMap(actor.MapID).SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.BUFF_CHANGE, null, actor, true);
        }
        #endregion
    }
