...
namespace SagaMap.Skill.SkillDefinations.Bard
{
    ...
    public class Classic : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            DefaultBuff skill = new DefaultBuff(args.skill, sActor, "Music", 60000 * level);
            ...
            if (actor.skillsong != null)
            {
                ...
                foreach (Actor act in actors)
                {
                    if (act.type == ActorType.PC || act.type == ActorType.PARTNER)
                    {
                        ...
                        {
                            act.Status.Additions["Classic"].AdditionEnd();
                            act.Status.Additions.TryRemove("Classic", out _);
                        }
                    }
                }
            }
        ...
        private class Activator : MultiRunTask
        {
            ...
            void TimerEventHandler(Actor actor, DefaultBuff skill)
            {
                ...
                if (ranges >= 200)
                {
                    actor.Status.Additions["Classic"].AdditionEnd();
                    actor.Status.Additions.TryRemove("Classic",out _);
                }
            }
        }
        #endregion
