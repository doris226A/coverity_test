    ...
    public class Fusion : ISkill
    {
            ...
            if (actor.skillsong != null)
            {
                ...
                foreach (Actor act in actors)
                {
                    if (act.type == ActorType.PC || act.type == ActorType.PARTNER)
                    {
                        ...
                        {
                            act.Status.Additions["Fusion"].AdditionEnd();
                            act.Status.Additions.TryRemove("Fusion", out _);
                        }
                    }
                }
            }
        ...
        private class Activator : MultiRunTask
        {
            ...
            void TimerEventHandler(Actor actor, DefaultBuff skill)
            {
                ...
                if (ranges > 200)
                {
                    actor.Status.Additions["Fusion"].AdditionEnd();
                    actor.Status.Additions.TryRemove("Fusion", out _);
                }
            }
        }
        #endregion
