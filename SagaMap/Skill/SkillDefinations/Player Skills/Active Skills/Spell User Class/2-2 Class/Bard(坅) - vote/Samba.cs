    ...
    public class Samba : ISkill
    {
            ...
            if (actor.skillsong != null)
            {
                ...
                foreach (Actor act in actors)
                {
                    if (act.type == ActorType.PC || act.type == ActorType.PARTNER)
                    {
                        ...
                        {
                            act.Status.Additions["DefUpCircle"].AdditionEnd();
                            act.Status.Additions.TryRemove("DefUpCircle", out _);
                        }
                    }
                }
            }
        ...
        private class Activator : MultiRunTask
        {
            ...
            void TimerEventHandler(Actor actor, DefaultBuff skill)
            {
                ...
                if (ranges > 200)
                {
                    actor.Status.Additions["DefUpCircle"].AdditionEnd();
                    actor.Status.Additions.TryRemove("DefUpCircle", out _);
                }
            }
        }
        #endregion
