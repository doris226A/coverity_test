    ...
    public class PopMusic : ISkill
    {
            ...
            if (actor.skillsong != null)
            {
                ...
                foreach (Actor act in actors)
                {
                    if (act.type == ActorType.PC || act.type == ActorType.PARTNER)
                    {
                        ...
                        {
                            act.Status.Additions["PopMusic"].AdditionEnd();
                            act.Status.Additions.TryRemove("PopMusic", out _);
                        }
                    }
                }
            }
        ...
        private class Activator : MultiRunTask
        {
            ...
            void TimerEventHandler(Actor actor, DefaultBuff skill)
            {
                ...
                if (ranges > 200)
                {
                    actor.Status.Additions["PopMusic"].AdditionEnd();
                    actor.Status.Additions.TryRemove("PopMusic", out _);
                }
            }
        }
        #endregion
