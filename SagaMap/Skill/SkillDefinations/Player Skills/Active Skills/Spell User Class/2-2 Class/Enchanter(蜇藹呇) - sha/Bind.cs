...
namespace SagaMap.Skill.SkillDefinations.Enchanter
{
    ...
    public class Bind : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            if (map.Info.earth[args.x, args.y] == 0)
            {
                MapClient.FromActorPC((ActorPC)sActor).SendSystemMessage("無法在指定的座標使用");
                return;
            }
            //创建设置型技能技能体
                ...
                try
                {
                    if (lifetime > 0)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                if (SkillHandler.Instance.CheckValidAttackTarget(actor.Caster, i))
                                {
                                    if (SkillHandler.Instance.CanAdditionApply(actor.Caster, i, SkillHandler.DefaultAdditions.鈍足, rate))
                                    {
