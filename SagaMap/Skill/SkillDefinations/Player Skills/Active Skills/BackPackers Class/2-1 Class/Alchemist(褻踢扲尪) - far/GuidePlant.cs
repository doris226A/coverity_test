        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            foreach (Actor act in affected)
            {
                if (act.type == ActorType.MOB)
                {
                    ...
                    if (m.BaseData.mobType == SagaDB.Mob.MobType.PLANT_MATERIAL || m.BaseData.mobType == SagaDB.Mob.MobType.TREE_MATERIAL)
                    {
                        if (m.HP <= 0) continue;
                        i++;
            ...
            foreach (Actor act in affected)
            {
                if (act.type == ActorType.MOB)
                {
                    ...
                    if (mob.BaseData.mobType == MobType.PLANT_MATERIAL || mob.BaseData.mobType == MobType.TREE_MATERIAL)
                    {
                        check = true;
                        Packets.Server.SSMG_NPC_NAVIGATION p = new SagaMap.Packets.Server.SSMG_NPC_NAVIGATION();
            ...
            if (!check)
            {
                MapClient.FromActorPC((ActorPC)sActor).SendSystemMessage("範圍內找不到植物");
                Packets.Server.SSMG_NPC_NAVIGATION_CANCEL p = new SagaMap.Packets.Server.SSMG_NPC_NAVIGATION_CANCEL();
                MapClient.FromActorPC((ActorPC)sActor).netIO.SendPacket(p);
            }*/
