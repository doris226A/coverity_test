        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            if (dActor.type == ActorType.PC)
            {
                ...
                if (SagaLib.Global.Random.Next(0, 99) < dePossessionRate)
                {
                    ...
                    if (actor.Inventory.Equipments.ContainsKey(SagaDB.Item.EnumEquipSlot.UPPER_BODY))
                    {
                        SagaMap.Network.Client.MapClient client = SagaMap.Network.Client.MapClient.FromActorPC(actor);
                        SagaDB.Item.Item item = actor.Inventory.Equipments[SagaDB.Item.EnumEquipSlot.UPPER_BODY];
