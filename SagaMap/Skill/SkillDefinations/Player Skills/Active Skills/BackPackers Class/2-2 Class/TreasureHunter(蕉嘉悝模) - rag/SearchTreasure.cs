    ...
    public class SearchTreasure : ISkill
    {
        ...
        public void Proc(Actor sActor, Actor dActor, SkillArg args, byte level)
        {
            ...
            foreach (Actor act in affected)
            {
                if (act.type == ActorType.MOB)
                {
                    ...
                    if (m.BaseData.mobType == SagaDB.Mob.MobType.TREASURE_BOX_MATERIAL || m.BaseData.mobType == SagaDB.Mob.MobType.CONTAINER_MATERIAL || m.BaseData.mobType == SagaDB.Mob.MobType.TIMBER_BOX_MATERIAL)
                    {
                        ...
                        /*if (SagaLib.Global.PosX16to8(act.X, map.Width) <= arrX && SagaLib.Global.PosY16to8(act.Y, map.Height) <= arrY)
                        {
                            if(map.GetLengthD(sActor.X, sActor.Y, SagaLib.Global.PosX16to8(act.X, map.Width), SagaLib.Global.PosY16to8(act.Y, map.Height)) <= length){
                                length = map.GetLengthD(sActor.X, sActor.Y, SagaLib.Global.PosX16to8(act.X, map.Width), SagaLib.Global.PosY16to8(act.Y, map.Height));
            ...
            else
            {
                ...
                //p.Y = SagaLib.Global.PosY16to8(arrY, map.Height);
                p.X = arrX;
                p.Y = arrY;
                //p.Type = 0;
