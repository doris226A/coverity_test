    ...
    public class MarionetteHarmony : ISkill
    {
        ...
        public int TryCast(ActorPC sActor, Actor dActor, SkillArg args)
        {
            if (sActor.type == ActorType.PC)
            {
                ...
                if (pc.TInt["MarioCtrl"] != 0)
                {
                    if (pc.TInt["MarioCtrl"] == 10050700 && args.skill.Level == 1)//零式泰迪.无属性
                        return 0;
                    else if (pc.TInt["MarioCtrl"] == 10021700 && args.skill.Level == 5)//火焰凤凰.火属性
                        return 0;
                    else if (pc.TInt["MarioCtrl"] == 10019300 && args.skill.Level == 3)//冰精灵.水属性
                        return 0;
                    else if (pc.TInt["MarioCtrl"] == 10030001 && args.skill.Level == 4)//电路机械.风属性
                        return 0;
                    else if (pc.TInt["MarioCtrl"] == 10027000 && args.skill.Level == 2)//皮诺.地属性
                        return 0;
                    else
                        return -12;
                }
        ...
        {
            SagaMap.Network.Client.MapClient.FromActorPC((ActorPC)sActor).MarionetteActivate((uint)sActor.TInt["MarioCtrl"]);
            int lifetime = 180000;
            ...
            if (sActor.Status.Additions.ContainsKey("MarioCtrl"))
            {
                sActor.Status.Additions["MarioCtrl"].AdditionEnd();
                sActor.Status.Additions.TryRemove("MarioCtrl", out _);
            }
        }
