...
namespace SagaMap.Skill.SkillDefinations.Striker
{
    ...
    public class ArrowGroove : ISkill 
    {
        ...
        protected void ProcSub(Actor sActor, Actor dActor, SkillArg args, byte level, SagaLib.Elements element)
        {
            ...
            new ArrowGroove.Activator(sActor, actor, args, path, element).Activate();
        }
        ...
        private class Activator : MultiRunTask
        {
            ...
            public Activator(SagaDB.Actor.Actor caster, ActorSkill actor, SkillArg args, List<MapNode> path, Elements element)
            {
                ...
                this.map = Manager.MapManager.Instance.GetMap(actor.MapID);
                this.period = 200;
                this.dueTime = 200;
                this.path = path;
                ...
                try
                {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            {
                                 Additions.Global.Stiff addition = new Stiff(skill.skill, actor, 400);
                                SkillHandler.ApplyAddition(actor, addition);
                                ...
                                if (actor.type == ActorType.MOB || actor.type == ActorType.PET || actor.type == ActorType.SHADOW)
                                    ((ActorEventHandlers.MobEventHandler)actor.e).AI.OnPathInterupt();
                            }
                            this.skill.affectedActors.Clear();
                            Singleton<SkillHandler>.Instance.PhysicalAttack(this.caster, dActor1, this.skill, this.element, this.factor);
