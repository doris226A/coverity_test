    ...
    public class VenomBlastSeq : ISkill
    {
        ...
        private class Activator : MultiRunTask
        {
            ...
            public Activator(Actor caster, ActorSkill actor, SkillArg args, byte level)
            {
                ...
                if (pc.Skills2_2.ContainsKey(2362) || pc.DualJobSkill.Exists(x => x.ID == 2362))
                {
                    if (pc.DualJobSkill.Exists(x => x.ID == 2362))
                        duallv = pc.DualJobSkill.FirstOrDefault(x => x.ID == 2362).Level;

                    //这里取主职的毒雾等级
                    var mainlv = 0;
                    if (pc.Skills2_2.ContainsKey(2362))
                        mainlv = pc.Skills2_2[2362].Level;

                    //这里取保留技能中的毒雾等级
                    var Reservelv = 0;
                    if (pc.SkillsReserve.ContainsKey(2362))
                        Reservelv = pc.SkillsReserve[2362].Level;
