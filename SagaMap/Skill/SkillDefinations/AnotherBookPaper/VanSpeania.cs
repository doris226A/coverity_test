...
namespace SagaMap.Skill.SkillDefinations.AnotherBookPaper
{
    ...
    /// ヴァン・スピアニア
    /// </summary>
    public class VanSpeania : ISkill
    {
        ...
        private class Activator : MultiRunTask
        {
            ...
            public Activator(Actor caster, ActorSkill actor, Actor dActor, SkillArg args, byte level)
            {
                ...
                lifetime = 500;//持续时间
                factor = new float[] { 0, 55.0f, 56.0f, 57.0f, 60.0f, 62.0f, 64.0f, 70.0f, 73.0f, 76.0f, 85.0f, 89.0f, 93.0f, 105.0f, 100.0f, 125.0f }[level];
            ...
            public override void CallBack()
            {
                ...
                try
                {
                    if (count < countMax)
                    {
                        ...
                        List<Actor> actors = Manager.MapManager.Instance.GetMap(caster.MapID).GetActorsArea(dActor, 100, true);
                        foreach (Actor i in actors)
                        {
                            if (SkillHandler.Instance.CheckValidAttackTarget(caster, i))
                            {
                                int damage1 = SkillHandler.Instance.CalcDamage(true, caster, i, skill, SkillHandler.DefType.MDef, SagaLib.Elements.Neutral, 0, factor);
                                int damage2 = SkillHandler.Instance.CalcDamage(false, caster, i, skill, SkillHandler.DefType.MDef, SagaLib.Elements.Neutral, 0, factor);
