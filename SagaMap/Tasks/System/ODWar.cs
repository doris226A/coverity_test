                    ...
                    if (ODWarManager.Instance.IsDefence(i.MapID))
                    {
                        if (i.StartTime.ContainsKey((int)now.DayOfWeek))
                        {
                            ...
                            if (map != null)
                            {
                                if (ODWarManager.Instance.IsDefence(i.MapID))
                                {
                                    ODWarManager.Instance.ShowWRPRanking(i.MapID);
                                    if ((now.Hour + 1 == time) && now.Minute >= 45 && (now.Minute % 5 == 0))
                                    {
                                        MapClientManager.Instance.Announce(string.Format(LocalManager.Instance.Strings.ODWAR_PREPARE, map.Name, (60 - now.Minute + 15)));//ECOKEY 時間修正
                                    ...
                                    if (now.Hour == time && now.Minute >= 1 && i.Started)
                                    {
                                        ...
                                        if (now.Minute == 44 || now.Minute == 45)
                                        {
                                            ODWarManager.Instance.SpawnMob(i.MapID, true);
                                        }
                                    }
                                    if (now.Hour == time && (now.Minute == 44) && i.Started)
                                    {
                                        ODWarManager.Instance.SpawnBoss(i.MapID);
                                    }
                                    if (now.Hour == time && (now.Minute == 46) && i.Started)
                                    {
                                        i.Started = false;
