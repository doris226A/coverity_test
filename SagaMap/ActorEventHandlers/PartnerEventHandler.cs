...
namespace SagaMap.ActorEventHandlers
{
    public class PartnerEventHandler : ActorEventHandler
    {
        ...
        public void OnActorAppears(Actor aActor)
        {
            ...
            if (aActor.ActorID == this.partner.Owner.ActorID && this.partner.type != ActorType.SHADOW)
            {
                if (!this.AI.Hate.ContainsKey(aActor.ActorID))
                    this.AI.Hate.TryAdd(aActor.ActorID, 1);
                else
                    this.AI.Hate.TryRemove(aActor.ActorID, out _);//二哈更改Hate
            }
        }
        ...
        {
            partner.Owner.TInt["MarryX_dog"] = 0;//ECOKEY 活動二哈背行李判斷
            partner.Owner.TInt["MarryX_Car"] = 0;//ECOKEY 活動二哈背行李判斷
        ...
        {
            //ECOKEY 如果主人坐著，直接更改位置紀錄
            if (partner.Owner.Motion == MotionType.SIT)
            {
        ...
        {
            //ECOKEY 寵物更新HPMPSP
            if (currentPC != null && sActor.type == ActorType.PARTNER)
            {
                MapClient.FromActorPC(currentPC).SendPartnerHPMPSP(sActor);
            }
