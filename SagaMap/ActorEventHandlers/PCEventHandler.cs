...
namespace SagaMap.ActorEventHandlers
{
    public class PCEventHandler : ActorEventHandler
    {
                ...
                case ActorType.GOLEM:
                    {
                        ...
                        if (actor.GolemType == GolemType.Sell)
                        {
                            if (actor.SellShop.Count != 0)
                            {
                                foreach (var key in actor.SellShop.Keys)
                                {
                                    var item = actor.Owner.Inventory.GetItem(key);
                                    //已经不存在的道具将从贩售商店移除
                                    if (item == null)
                                        actor.SellShop.Remove(key);
                                    //已经不存在的道具将从贩售商店移除
                                    if (item.Stack == 0)
                                        actor.SellShop.Remove(key);
                                    else if (actor.SellShop[key].Count > item.Stack)
                                        actor.SellShop[key].Count = item.Stack;
                                    //变形的道具将从贩售商店移除
        ...
        public void OnActorDisappears(Actor dActor)
        {
            if (Client == null) return;
            //ECOKEY 四重奏error
            lock (Client.Character.VisibleActors)
            {
            ...
            try
            {
                ...
                switch (arg.argType)
                {
                    case SkillArg.ArgType.Item_Cast:
                        ...
                        p2.SkillID = (ushort)arg.item.ActiveSkillID;
        ...
        public void OnDie()
        {
            ...
            /*if (this.Client.Character.TimerItems.Count != 0)
            {
                MapServer.charDB.SaveTimerItem(this.Client.Character);
                MapServer.charDB.GetTimerItem(this.Client.Character);
                uint idx = 0;
                foreach (var item in this.Client.Character.TimerItems)
                {
                    ...
                    var type = methods.Invoke(new object[5] { null, this.Client.Character, buffname, (int)(item.Value.Duration * 1000), 1000 }) as Addition;
                    ...
                    Skill.SkillHandler.ApplyAddition(this.Client.Character, buff);
            ...
            */
            ...
            Manager.KnightWarManager.Instance.KnightWarDie(this.Client.Character);//ECOKEY 新增騎士團死亡判斷
            ...
            {
                //ECOKEY 暗黑騎士的自動復活
                if (this.Client.Character.Skills2_2.ContainsKey(957))
                    this.Client.Character.TInt["DarkKnightRevive"] = this.Client.Character.Skills2_2[957].Level;

                this.Client.Character.BattleStatus = 0;
                this.Client.SendChangeStatus();
                this.Client.Character.TInt["Revive"] = 5;
                this.Client.EventActivate(0xF1000000);
                return;//ECOKEY 避免執行到青蛙
            }
            //ECOKEY 青蛙復活1214修正
            if (Client.currentEvent == null && Client.scriptThread == null && SagaMap.Skill.SkillHandler.Instance.CountItem(this.Client.Character, 16002110) >= 1)
            {
                this.Client.Character.Buff.TurningPurple = true;
                this.Client.Map.SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.BUFF_CHANGE, null, this.Client.Character, true);
                this.Client.Character.BattleStatus = 0;
                this.Client.SendChangeStatus();
                this.Client.Character.TInt["Revive"] = 6;
                this.Client.EventActivate(0xF1000000);
            }
        ...
        public void OnAttack(Actor aActor, MapEventArgs args)
        {
            ...
            //ECOKEY 坐下優化
        /*    if (Client.Character.Tasks.ContainsKey("Regeneration"))
            {
                Client.Character.Tasks["Regeneration"].Deactivate();
                Client.Character.Tasks.Remove("Regeneration");
            }
            if (Client.Character.Buff.Sit == true)
            {
                Client.Character.Buff.Sit = false;
                SagaMap.Map.BuffChangeEventArgs buffChangeEventArgs = new SagaMap.Map.BuffChangeEventArgs(Client.Character);
                Client.map.SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.BUFF_CHANGE, buffChangeEventArgs, Client.Character, true);
            }*/
        }
        ...
        public void OnPlayerMode(Actor aActor)
        {
            ...
            //ECOKEY 騎士團 防英雄狀態消失
            if (pc.Mode == PlayerMode.KNIGHT_EAST_HERO ||
        ...
        public void PropertyUpdate(UpdateEvent arg, int para)
        {
            ...
            switch (arg)
            {
                ...
                case UpdateEvent.VCASH_POINT:
                    ...
                    break;
                //ECOKEY NC
                case UpdateEvent.NC_POINT:
                    MapServer.charDB.SaveNCShop(Client.Character);
                    break;
        ...
        public void PropertyRead(UpdateEvent arg)
        {
            ...
            switch (arg)
            {
                case UpdateEvent.VCASH_POINT:
                    ...
                    //ECOKEY NC
                case UpdateEvent.NC_POINT:
                    MapServer.charDB.GetNCShop(Client.Character);
                    break;
            }
        }
