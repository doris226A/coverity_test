...
namespace SagaMap.Scripting
{
    public abstract partial class Event
    {
        ...
        public void OpenRebirthReward(ActorPC pc)
        {
        ...
        public void OpenChoiceShop(ActorPC pc) //ECOKEY
        {
        ...
        /// <param name="durationType">0 = 分, 1 = 日</param>
        protected void ActiveTimerItem(ActorPC pc, uint ID, uint duration, byte durationType)
        {
            if (!ActorTimerItemFactory.Instance.Items.ContainsKey(ID))
                return;

            ActorTimerItem item = ActorTimerItemFactory.Instance.Items[ID];
            ...
            System.DateTime curr_time = TimeZone.CurrentTimeZone.ToLocalTime(new System.DateTime(1970, 1, 1));
            var endTime = 0u;

            /* if (Convert.ToBoolean(item.DurationType))
             {
                 endTime = (uint)(time - curr_time).Add(new TimeSpan(0, 0, (int)item.Duration, 0)).TotalSeconds;
             }
             else
             {
                 endTime = (uint)(time - curr_time).Add(new TimeSpan(0, 0, (int)item.Duration, 0)).TotalSeconds;
             ...
             /*if (Convert.ToBoolean(item.DurationType))
                 endTime = (uint)(time - curr_time).Add(new TimeSpan(int.Parse(item.Duration.ToString()), 0, 0, 0)).TotalSeconds;
             else
                 endTime = (uint)(time - curr_time).Add(new TimeSpan(0, int.Parse(item.Duration.ToString()), 0)).TotalSeconds;*/


            //把時間轉換成秒數
            if (item.DurationType == true)
            {
                //  item.Duration = duration;
                item.Duration = duration * 24 * 60 * 60;
            }
            else
            {
            ...
            if (pc.TimerItems.ContainsKey(item.BuffName))
                return;

            pc.TimerItems.Add(item.BuffName, item);
            MapServer.charDB.SaveTimerItem(pc);
            //ECOKEY 常駐Task，限時道具
            if (!pc.Tasks_Online.ContainsKey("TimerItem"))
            {
                Tasks.PC.TimerItem TimerItemTask = new SagaMap.Tasks.PC.TimerItem(pc);
                pc.Tasks_Online.Add("TimerItem", TimerItemTask);
                TimerItemTask.Activate();
            }
            /*var types = Type.GetType(item.BuffCodeName);
            var buffnames = item.BuffCodeName.Split('.');
            ...
            var type = methods.Invoke(new object[5] { null, pc, buffname, (int)((item.EndTime - item.StartTime) * 1000), 1000 }) as Addition;

            var buff = type as DefaultBuff;
            if (item.BuffValues.Length != 0)
            {
                var lstbonus = item.BuffValues.Split(';');
                foreach (var bonusentry in lstbonus)
                {
                    ...
                    {
                        if (buff.Variable.ContainsKey(item.BuffName))
                            buff.Variable.Remove(item.BuffName);

                        buff.Variable.Add(item.BuffName, int.Parse(bonusentry));
                    }
                }
            }

            Skill.SkillHandler.ApplyAddition(pc, buff);*/

            Packets.Server.SSMG_ITEM_SEND_TIMER pitem = new Packets.Server.SSMG_ITEM_SEND_TIMER();
            pitem.Slot = (uint)pc.TimerItems.Count();
            pitem.ItemID = ID;
            pitem.StartTime = startTime;
            pitem.EndTime = endTime;
            pitem.Unknow = 0xFF;
            client.netIO.SendPacket(pitem);
        }
        /// <summary>
        /// 永久顯示/隱藏NPC
        /// </summary>
        /// <param name="pc">玩家</param>
        /// /// <param name="Set">顯示True或隱藏False</param>
        /// <param name="mapid">NPC所在地圖</param>
        /// <param name="npcid">NPC的ID</param>
        protected void SetNpc(ActorPC pc, bool Set, uint mapid, uint npcid)
        {
            if (Set)
            {
                ...
                {
                    Dictionary<uint, bool> NPC = new Dictionary<uint, bool>() { { npcid, true } };
                    pc.NPCStates.Add(mapid, NPC);
                }
            }
            else
            {
                if (pc.NPCStates.ContainsKey(mapid))
                {
                    if (pc.NPCStates[mapid].ContainsKey(npcid))
                    {
                        pc.NPCStates[mapid].Remove(npcid);
                        if (pc.NPCStates[mapid].Count <= 0)
                        {
                            pc.NPCStates.Remove(mapid);
                        }
                    }
                }
                /*else
                {
                    Dictionary<uint, bool> NPC = new Dictionary<uint, bool>() { { npcid, false } };
                    pc.NPCStates.Add(mapid, NPC);
                }*/
            }
        ...
        /// </summary>
        /// <param name="pc">玩家</param>
        /// <param name="type">UI参数</param>
        protected void ShowHairPreview(ActorPC pc, int type = 0)
        {
            MapClient client = GetMapClient(pc);
        ...
        /// 召唤契约之证的UI窗口
        /// </summary>
        /// <param name="pc">当前玩家</param>
        /// <param name="partnername">伴侣名称</param>
        /// <param name="visable">窗口是否可见</param>
        protected void TheCertificateOfFettersWindowOpen(ActorPC pc, string partnername, uint visable = 1)
        {
            MapClient client = GetMapClient(pc);
            Packets.Server.SSMG_PLAYER_THE_CERIFICATE_OF_FETTERS_WINDOW_OPEN p = new Packets.Server.SSMG_PLAYER_THE_CERIFICATE_OF_FETTERS_WINDOW_OPEN();
            p.Visable = visable;
        ...
        protected void OpenShopByList(ActorPC pc, uint rate, ShopType type, params uint[] Goods)
        {
            ...
            {
                case ShopType.None:
                    p.Gold = (ulong)pc.Gold;
                    p.Bank = pc.Account.Bank;
                    finalrate = (uint)((float)finalrate * (float)((float)rate / 1000.0f) * (float)((float)pc.Status.buy_rate / 100.0f) * (float)((float)pc.Status.buy_rate_iris / 100.0f));
                    break;
                case ShopType.CP:
                    p.Gold = pc.CP;
        ...
        protected void OpenShopBuy(ActorPC pc, uint shopID)
        {
            ...
            switch (shop.ShopType)
            {
                case ShopType.None:
                    ...
                    if (shopID > 30000)
                    {
                        finalrate = (uint)((float)finalrate * (float)((float)shop.SellRate / 100.0f));
                        break;
                    }
                    else if (shopID < 29999)
                    {
                        finalrate = (uint)((float)finalrate * (float)((float)shop.SellRate / 100.0f) * (float)((float)pc.Status.buy_rate / 100.0f) * (float)((float)pc.Status.buy_rate_iris / 100.0f));
                        break;
                    }
                    break;
                case ShopType.CP:
        ...
        protected void OpenShopBuy(ActorPC pc)
        {
            ...
            uint finalrate = 1000;
            finalrate = (uint)((float)finalrate * (float)((float)pc.Status.buy_rate / 100.0f) * (float)((float)pc.Status.buy_rate_iris / 100.0f));
            p.Rate = finalrate;
            p.Goods = this.goods;
            p.Gold = (ulong)pc.Gold;
            p.Bank = pc.Account.Bank;
        ...
        protected void OpenShopSell(ActorPC pc, uint shopID)
        {
            ...
            MapClient client = GetMapClient(pc);
            uint finalrate = 1000;
            finalrate = (uint)((float)finalrate * (float)((float)shop.BuyRate / 100.0f) * (float)((float)pc.Status.sell_rate / 100.0f) * (float)((float)pc.Status.sell_rate_iris / 100.0f));
            p.Rate = finalrate;
        ...
        protected void OpenShopSell(ActorPC pc)
        {
            ...
            MapClient client = GetMapClient(pc);
            uint finalrate = 1000;
            finalrate = (uint)((float)finalrate * (float)((float)pc.Status.sell_rate / 100.0f) * (float)((float)pc.Status.sell_rate_iris / 100.0f));
            p.Rate = finalrate;
        ...
        protected SagaDB.Item.Item TakeItem(ActorPC pc, uint itemID, ushort count)
        {
            MapClient client = GetMapClient(pc);
            if (count <= 0)
                return client.DeleteItemID(itemID, 0, true);
        ...
        public void Synthese(ActorPC pc, ushort skillID, byte skillLv, bool noMoney)
        {
            ...
            //ECOKEY 精緻道具技能（修正NPC無法執行這段）
            if (skillID == 2009 && noMoney)
            {
                int s = Select(pc, "  ", "", "精製道具", "擴充裝備插槽", "升級卡片");
                switch (s)
                {
                    case 1:
                        break;
                    case 2:
                        //精緻道具技能不用錢
                        client.irisCardNoMoney = true;
                        SystemMessage(pc, "使用技能，無須費用");
                        //武器製作
                        if (pc.Skills2.ContainsKey(2010) || pc.Skills2_1.ContainsKey(2010) || pc.SkillsReserve.ContainsKey(2010))
                        {
                            SystemMessage(pc, "提高除了弓、杖以外武器機率");
                            client.irisCardArms = true;
                        }
                        //弓製作
                        if (pc.Skills.ContainsKey(2034) || pc.Skills2_1.ContainsKey(2034) || pc.SkillsReserve.ContainsKey(2034))
                        {
                            SystemMessage(pc, "提高弓機率");
                            client.irisCardBow = true;
                        }
                        //杖製作
                        if (pc.Skills2.ContainsKey(2021) || pc.Skills2_1.ContainsKey(2021) || pc.SkillsReserve.ContainsKey(2021))
                        {
                            SystemMessage(pc, "提高杖機率");
                            client.irisCardStaff = true;
                        }
                        //裁縫
                        if (pc.Skills.ContainsKey(2054) || pc.Skills2_1.ContainsKey(2054) || pc.SkillsReserve.ContainsKey(2054))
                        {
                            SystemMessage(pc, "提高項鍊機率");
                            client.irisCardNeck = true;
                        }
                        //防具製作
                        if (pc.Skills2.ContainsKey(2017) || pc.Skills2_1.ContainsKey(2017) || pc.SkillsReserve.ContainsKey(2017))
                        {
                            SystemMessage(pc, "提高防具機率");
                            client.irisCardArmor = true;
                        }
                        ItemAddSlot(pc);
                        return;
                    case 3:
                        client.irisCardNoMoney = true;
                        SystemMessage(pc, "使用技能，無須費用，提高機率");
                        IrisCardAssemble(pc);
                        return;
                }
            }
            ...
            foreach (uint i in client.syntheseItem.Keys)
            {
                ...
                if (complete && (pc.Gold >= info.Gold || noMoney))
                {
                    for (uint k = 0; k < client.syntheseItem[i]; k++)
                    {
                        ...
                        int maxRate = info.Products.Sum(x => x.Rate);
                        //bool givemax = false;
                        //Logger.ShowInfo("抽抽：" + ran.ToString());

                        foreach (ItemElement j in info.Products)
                        {
                            //Logger.ShowInfo("j.Rate：" + j.Rate.ToString());
                            /*if (ran == maxRate)
                            {
                                if (!givemax)
                                {
                            ...
                            if (ran >= baseValue && ran <= maxVlaue)//只改ran <= maxVlaue，避免東西被吃掉
                            {
                                if (sketch && mobID != 0)
                                {
                ...
                foreach (var item in result.Keys)
                {
                    ...
                    if (item == 0)
                        continue;
                    if (result[item][0].mobid != 0)
                    {
                        GiveItem(pc, item, (ushort)result[item].Sum(x => x.count), true, result[item][0].mobid);
                    }
                    else
                    {
                        var targetitem = ItemFactory.Instance.GetItem(item);
                        targetitem.ActiveSkillID = result[item][0].skillid;
                        if (targetitem.Stackable)
                        {
                            targetitem.Stack = (ushort)result[item].Sum(x => x.count);
                            GiveItem(pc, targetitem);
                        }
                        else
                        {
                            for (int idx = 0; idx < result[item].Count; idx++)
                            {
                                //ECOKEY 合成需重啟的問題
                                var targetitemA = ItemFactory.Instance.GetItem(item);
                                GiveItem(pc, targetitemA);
                            }
                        }
                    }
        ...
        protected void BankDeposit(ActorPC pc)
        {
            ...
            MapServer.accountDB.WriteUser(pc.Account);//ECOKEY 儲存銀行資料

            Logger logger = new Logger("玩家向銀行存錢.txt");
            string log = "\r\n玩家" + pc.Name + " 身上金錢：" + pc.Gold + "，在銀行存款扣掉了：" + amount + "，現在銀行總共：" + pc.Account.Bank + "  ";
            logger.WriteLog(log);
        }
        ...
        protected void BankWithdraw(ActorPC pc)
        {
            ...
            Logger logger = new Logger("玩家向銀行存錢.txt");
            string log = "\r\n玩家" + pc.Name + " 在銀行取了：" + amount + "，現在銀行總共：" + pc.Account.Bank + "，現有身上金錢：" + pc.Gold + "  ";
            logger.WriteLog(log);
        }
        ...
        /// </summary>
        /// <param name="pc">玩家</param>
        public void OpenTreasureBox1(ActorPC pc, bool timber, bool treasure, bool container)
        {
            List<SagaDB.Item.Item> inv = pc.Inventory.GetContainer(ContainerType.BODY);
            List<SagaDB.Item.Item> list;
            var box =
            ...
            int totalBoxesToOpen = Math.Min(list.Count, 50);

            if (totalBoxesToOpen > 0)
            {
                string[] names = new string[totalBoxesToOpen];
                int i = 0;

                for (int sel = 0; sel < totalBoxesToOpen; sel++)
                {
                    SagaDB.Item.Item item = list[sel];
                    names[i] = item.BaseData.name;
                    i++;
                    ...
                    //ECOKEY 修復無限開箱BUG
                    if (CountItem(pc, item.ItemID) < 1)
                    {
                        GetMapClient(pc).SendSystemMessage("找不到箱子");
                        return;
                    }
                ...
                MapClient client = GetMapClient(pc);
                client.SendSystemMessage($"You have opened {totalBoxesToOpen} boxes.");
            }
            else
            {
                MapClient client = GetMapClient(pc);
        ...
        public void OpenTreasureBox(ActorPC pc, bool timber, bool treasure, bool container)
        {
            ...
            {
                MapClient client = GetMapClient(pc);
                client.SendSystemMessage("你身上太多箱了,請不要超過20個箱");
                return;
            }
        ...
        /// </summary>
        /// <param name="pc">玩家</param>
        public void OpenTreasureBox2(ActorPC pc, bool timber, bool treasure, bool container)
        {
            List<SagaDB.Item.Item> inv = pc.Inventory.GetContainer(ContainerType.BODY);
            List<SagaDB.Item.Item> list;
            var box =
            ...
            {
                MapClient client = GetMapClient(pc);
                client.SendSystemMessage("你身上太多箱了,請不要超過50個箱");
                return;
            }
        ...
        /// </summary>
        /// <param name="pc">玩家</param>
        protected void ExitFGarden(ActorPC pc) //ECOKEY
        {
            Map map = MapManager.Instance.GetMap(pc.MapID);
            pc.Speed = 350;
            if (map.IsMapInstance && map.OriID == 70000000)
            {
        ...
        public void NCShopOpen(ActorPC pc)
        {
            ...
            p.CurrentPoint = (uint)pc.NCShopPoints;
            p.type = 1;
        ...
        protected void WarpToDungeon(ActorPC pc)
        {
            ...
            if (pc.PossesionedActors.Count != 0)
            {
                ...
                foreach (ActorPC i in possessioned)
                {
                    ...
                    if (i.Party != null && pc.Party != null)
                    {
                        if (pc.Party.ID != i.Party.ID)
                        {
                            Packets.Client.CSMG_POSSESSION_CANCEL p = new SagaMap.Packets.Client.CSMG_POSSESSION_CANCEL();
                            p.PossessionPosition = i.PossessionPosition;
        ...
        protected bool UseStamp1(ActorPC pc, StampGenre genre, StampSlot slot)
        {
            if (!pc.Stamp[genre].Test(slot))
            {
                pc.Stamp[genre].SetValue(slot, true);
        ...
        protected bool ItemEnhance(ActorPC pc, uint price)
        {
            var res = from item in pc.Inventory.GetContainer(ContainerType.BODY)
                      where ((item.IsArmor || item.IsWeapon || item.BaseData.itemType == ItemType.SHIELD || item.BaseData.itemType == ItemType.ACCESORY_NECK) && item.Refine < 30 && item.PossessionOwner == null && item.PossessionedActor == null)
                      select item;
        ...
        protected bool ItemEnhanceDEM(ActorPC pc, uint price)
        {
            var res = from item in pc.Inventory.GetContainer(ContainerType.BODY)
                      where ((item.BaseData.itemType == ItemType.PARTS_BODY ||
                                item.BaseData.itemType == ItemType.PARTS_BLOW ||
                                ...
                                item.BaseData.itemType == ItemType.SETPARTS_LONGRANGE) && item.Refine < 30 && item.PossessionOwner == null && item.PossessionedActor == null)
                      select item;
            List<SagaDB.Item.Item> items = res.ToList();
            ...
            if (items.Count > 0)
            {
                Packets.Server.SSMG_DEM_ITEM_ENHANCE_LIST p = new SagaMap.Packets.Server.SSMG_DEM_ITEM_ENHANCE_LIST();
                p.Items = items;
                MapClient client = MapClient.FromActorPC(pc);
                client.netIO.SendPacket(p);
        ...
        /// <param name="pc">玩家</param>
        /// <param name="price">金額</param>
        protected bool ItemElementsEnhanceDEM(ActorPC pc, uint price)
        {
            var res = from item in pc.Inventory.GetContainer(ContainerType.BODY)
                      where ((item.BaseData.itemType == ItemType.PARTS_BODY ||
                                item.BaseData.itemType == ItemType.PARTS_BLOW ||
            ...
            {
                //火屬性強化の結晶
                if (CountItem(pc, 90000047) > 0)
                    if (!items.Exists(x => x.ItemID == 90000047))
                        items.AddRange(GetItem(pc, 90000047));
                //水屬性強化の結晶
                if (CountItem(pc, 90000048) > 0)
                    if (!items.Exists(x => x.ItemID == 90000048))
                        items.AddRange(GetItem(pc, 90000048));
                //風屬性強化の結晶
                if (CountItem(pc, 90000049) > 0)
                    if (!items.Exists(x => x.ItemID == 90000049))
                        items.AddRange(GetItem(pc, 90000049));
                //土屬性強化の結晶
                if (CountItem(pc, 90000050) > 0)
                    if (!items.Exists(x => x.ItemID == 90000050))
                        items.AddRange(GetItem(pc, 90000050));
                //光屬性強化の結晶
                if (CountItem(pc, 90000051) > 0)
                    if (!items.Exists(x => x.ItemID == 90000051))
                        items.AddRange(GetItem(pc, 90000051));
                //闇屬性強化の結晶
                if (CountItem(pc, 90000052) > 0)
                    if (!items.Exists(x => x.ItemID == 90000052))
                        items.AddRange(GetItem(pc, 90000052));
        ...
        protected void ItemFusionDEM(ActorPC pc, string confirm)
        {
            ...
            do
            {
                Packets.Server.SSMG_DEM_ITEM_FUSION p = new SagaMap.Packets.Server.SSMG_DEM_ITEM_FUSION();
                    ...
                    if (effectItem != null && viewItem != null)
                    {
                        ...
                        switch (sel)
                        {
                            ...
                            case 3:
                                if (pc.Gold >= price)
                                {
                                    SagaMap.Packets.Server.SSMG_ITEM_FUSION_RESULT.FusionResult res = checkFusionItem(viewItem, effectItem);
                                    p2.Result = res;
                                    pc.Gold -= price;
                                    if (Global.Random.Next(0, 99) < rate)
                                    {
                                        viewItem.PictID = effectItem.BaseData.imageID;
                                        viewItem.HP += effectItem.BaseData.hp;
                                        viewItem.MP += effectItem.BaseData.mp;
                                        viewItem.SP += effectItem.BaseData.sp;
                                        viewItem.Atk1 += effectItem.BaseData.atk1;
                                        viewItem.Atk2 += effectItem.BaseData.atk2;
                                        viewItem.Atk3 += effectItem.BaseData.atk3;
                                        viewItem.MAtk += effectItem.BaseData.matk;
                                        viewItem.Def += effectItem.BaseData.def;
                                        viewItem.MDef += effectItem.BaseData.mdef;
                                        viewItem.AvoidMelee += effectItem.BaseData.avoidMelee;
                                        viewItem.AvoidCritical += effectItem.BaseData.avoidCritical;
                                        viewItem.AvoidMagic += effectItem.BaseData.avoidMagic;
                                        viewItem.AvoidRanged += effectItem.BaseData.avoidRanged;
                                        viewItem.HitMelee += effectItem.BaseData.hitMelee;
                                        viewItem.HitCritical += effectItem.BaseData.hitCritical;
                                        viewItem.HitMagic += effectItem.BaseData.hitMagic;
                                        viewItem.HitRanged += effectItem.BaseData.hitRanged;
                                        viewItem.Str += effectItem.BaseData.str;
                                        viewItem.Dex += effectItem.BaseData.dex;
                                        viewItem.Int += effectItem.BaseData.intel;
                                        viewItem.Vit += effectItem.BaseData.vit;
                                        viewItem.Agi += effectItem.BaseData.agi;
                                        viewItem.Mag += effectItem.BaseData.mag;
                                        TakeItemBySlot(pc, effectItem.Slot, 1);
                                        client.SendItemInfo(viewItem);
                                        ShowEffect(pc, 5191);
                        ...
                        switch (sel)
                        {
                            ...
                            case 3:
                                if (pc.Gold >= price)
                                {
                                    ...
                                    if (Global.Random.Next(0, 99) < rate)
                                    {
                                        //ECOKEY 修復外觀複製品和冰淇淋邏輯
                                        if (viewItem.PictID != 0)
                                        {
                                            effectItem.PictID = viewItem.PictID;
                                            if (effectItem.ActorPartnerID != 0)
                                                effectItem.ActorPartnerID = viewItem.ActorPartnerID;
                                            else
                                                effectItem.ActorPartnerID = viewItem.PictID;
                                        }
                                        else
                                        {
                                            effectItem.PictID = viewItem.BaseData.imageID;
                                            effectItem.ActorPartnerID = viewItem.BaseData.id;
                                        }
                                        TakeItemBySlot(pc, viewItem.Slot, 1);
                                        client.SendItemInfo(effectItem);
                                        ShowEffect(pc, 5191);
                                    }
                                    else//ECOKEY 修復失敗實顯示成功的問題
                                    {
                                        p2.Result = SagaMap.Packets.Server.SSMG_ITEM_FUSION_RESULT.FusionResult.FAILED;
                                    }
        ...
        /// </summary>
        /// <param name="pc">玩家</param>
        protected void ItemBreak(ActorPC pc)
        {
            MapClient client = MapClient.FromActorPC(pc);
            int sel = Select(pc, "選擇方案", "", "50萬G（100%成功）", "10萬G（30%成功）", "取消");
            switch (sel)
            {
                case 1:
                    client.itemBreak_price = 500000;
                    client.itemBreak_rate = 100;
                    break;
                case 2:
                    client.itemBreak_price = 100000;
                    client.itemBreak_rate = 30;
                    break;
                case 3:
                    return;
            }
            ...
            var it =
                ...
                where c.PictID != 0 && c.BaseData.itemType != ItemType.PARTNER &&
                                        c.BaseData.itemType != ItemType.RIDE_PARTNER &&
                                        c.BaseData.itemType != ItemType.RIDE_PET &&
                                        c.BaseData.itemType != ItemType.RIDE_PET_ROBOT &&
                                        c.BaseData.itemType != ItemType.PET
                select c;

            SagaMap.Packets.Server.SSMG_ITEM_BREAK p = new SagaMap.Packets.Server.SSMG_ITEM_BREAK();
            p.Pets = it.ToList();
            MapClient.FromActorPC(pc).netIO.SendPacket(p);
        ...
        protected void NPCChangeView(ActorPC pc, uint npcID, uint pictID)
        {

            Packets.Server.SSMG_NPC_SHOWPICT_VIEW p = new SagaMap.Packets.Server.SSMG_NPC_SHOWPICT_VIEW();
            p.NPCID = npcID;
            p.PictID = pictID;
            MapClient.FromActorPC(pc).netIO.SendPacket(p);
        ...
        /// <param name="y">Y</param>
        /// <param name="dir">方向</param>
        public void NPCSetting(ActorPC pc, uint npcID, byte x, byte y, byte dir)
        {
            Packets.Server.SSMG_NPC_SHOWPICT_LOCATION p = new Packets.Server.SSMG_NPC_SHOWPICT_LOCATION();
            p.NPCID = npcID;
            p.X = x;
        ...
        /// </summary>
        /// <param name="pc">玩家</param>
        /// <param name="type">1：全部隐藏，2：只隐藏其他玩家，3：只隐藏自己</param>
        public void HideActors(ActorPC pc, byte type)
        {
            MapClient client = GetMapClient(pc);
            Packets.Server.SSMG_NPC_ACTOR_HIDE p = new Packets.Server.SSMG_NPC_ACTOR_HIDE();
            p.Appear = 2;
            switch (type)
            {
                case 1:
        ...
        protected void ItemAddSlot(ActorPC pc)
        {
            ...
            foreach (SagaDB.Item.Item i in pc.Inventory.GetContainer(ContainerType.BODY))
            {
                if (i.IsEquipt)
                {
                    if (i.CurrentSlot >= 10 || (i.EquipSlot.Contains(EnumEquipSlot.CHEST_ACCE) && i.CurrentSlot >= 5) || i.PossessionedActor != null || i.PossessionOwner != null)
                        continue;
            ...
            */
            if (items.Count > 0)
            {
                ...
                //スロット追加の奧義
                if (CountItem(pc, 16022400) > 0)
                    items.Add(GetItem(pc, 16022400)[0].Slot);
        ...
        protected void FGardenChangeWeather(ActorPC pc, byte weather)
        {
            ...
            if (map.OriID == 70000000)
            {
                foreach (Actor i in map.Actors.Values)
                {
                    if (i != null && i.type == ActorType.PC && ((ActorPC)i).Online)
                    {
                        Packets.Server.SSMG_FG_CHANGE_WEATHER p = new SagaMap.Packets.Server.SSMG_FG_CHANGE_WEATHER();
                        p.Weather = weather;
                        MapClient.FromActorPC(pc).netIO.SendPacket(p);
                ...
                if (map.Creator != null)
                {
                    ((ActorPC)map.Creator).AInt["FGarden_Weather"] = weather;
                }
            }
        }
            ...
            if (map.OriID == 70000000)
            {
                foreach (Actor i in map.Actors.Values)
                {
                    if (i != null && i.type == ActorType.PC && ((ActorPC)i).Online)
                    {
                        Packets.Server.SSMG_FG_CHANGE_SKY p = new SagaMap.Packets.Server.SSMG_FG_CHANGE_SKY();
                        p.Sky = sky;
                        MapClient.FromActorPC((ActorPC)i).netIO.SendPacket(p);
                    }
                }
                if (map.Creator != null)
                {
                    ((ActorPC)map.Creator).AInt["FGarden_Heavenly"] = sky;
                }
            }
        }
        ...
        protected void PetRecover(ActorPC pc, int amount)//ECOKEY 修復寵物太多會卡住的問題，最多11隻
        {
            var it =
                from c in pc.Inventory.GetContainer(ContainerType.BODY)
                where (c.BaseData.itemType == ItemType.PET || c.BaseData.itemType == ItemType.PET_NEKOMATA || c.BaseData.itemType == ItemType.RIDE_PET || c.BaseData.itemType == ItemType.RIDE_PET_ROBOT || c.BaseData.itemType == ItemType.PARTNER || c.BaseData.itemType == ItemType.RIDE_PARTNER) &&
                    c.Durability < c.BaseData.durability
                select c;
            List<SagaDB.Item.Item> petlist = it.ToList();
            if (it.ToList().Count >= 12) petlist.RemoveRange(11, petlist.Count - 11);
            ...
            if (client.selectedPet != uint.MaxValue)
            {
                ...
                if (item != null)
                {
                    if (item.BaseData.itemType == ItemType.PET || item.BaseData.itemType == ItemType.PET_NEKOMATA || item.BaseData.itemType == ItemType.RIDE_PET || item.BaseData.itemType == ItemType.RIDE_PET_ROBOT || item.BaseData.itemType == ItemType.PARTNER || item.BaseData.itemType == ItemType.RIDE_PARTNER)
                    {
                        ...
                        client.SendItemInfo(item);
                        //ECOKEY 一天一次回復
                        pc.AStr["petRecover"] = DateTime.Today.ToString("d");
                    }
                }
            }
        }
        ...
        protected void DEMIC(ActorPC pc)
        {
            ...
            p.Alpha = (byte)(pc.TimerItems.ContainsKey("COST_LIMIT_ALPHA") ? 1 : 0);//ECOKEY DEM多的面板A
            p.Beta = (byte)(pc.TimerItems.ContainsKey("COST_LIMIT_BETA") ? 1 : 0);//ECOKEY DEM多的面板B
            p.CL = pc.CL;
            client.netIO.SendPacket(p);
            foreach (byte i in pc.Inventory.DemicChips.Keys)
            {
                ...
                p2.Page = i;
                //p2.Size = (byte)pc.TInt["AA"];
                //p2.Size = (byte)panelCount(pc, i, false);
                //ECOKEY DEM修復
                p2.Size = 80;
        ...
        /// </summary>
        /// <param name="pc">玩家</param>
        /// <param name="newitemID">新寵物道具ID</param>
        protected void PetChangeItem(ActorPC pc, uint newitemID, string newname)
        {
            MapClient client = MapClient.FromActorPC(pc);
            if (pc.Inventory.Equipments.ContainsKey(EnumEquipSlot.PET))
            {
                GiveItem(pc, newitemID, 1);
                SagaDB.Item.Item partneritem = pc.Inventory.Equipments[SagaDB.Item.EnumEquipSlot.PET];
                List<SagaDB.Item.Item> pets = GetItem(pc, newitemID);
                SagaDB.Item.Item pet = pets[0];
                foreach (SagaDB.Item.Item i in pets)
                {
                    if (i.ActorPartnerID == 0)
                    {
                        pet = i;
                        break;
                    }
                ...
                {
                    pc.Partner.Name = newname;
                    SagaMap.MapServer.charDB.SavePartner(pc.Partner);
                    pet.ActorPartnerID = partneritem.ActorPartnerID;
                }

                SagaDB.Partner.PartnerData newpet = SagaDB.Partner.PartnerFactory.Instance.GetPartnerData(pet.BaseData.petID);
                foreach (ushort nowList2 in newpet.born_active_cubes)
                {
                    if (nowList2 != 0 && !pc.Partner.equipcubes_activeskill.Contains(nowList2))
                    {
                        pc.Partner.equipcubes_activeskill.Add(nowList2);
                        MapServer.charDB.SavePartnerCube(pc.Partner);
                    }
                }

                foreach (ushort nowList in newpet.born_skills)
                {
                    if (nowList != 0 && !pc.Partner.equipcubes_action.Contains(nowList))
                    {
                        pc.Partner.equipcubes_action.Add(nowList);
                        MapServer.charDB.SavePartnerCube(pc.Partner);
                    }
                }
        ...
        {
            if ((byte)job % 10 != 7 && (byte)job != 120)//ECOKEY 修正小丑轉職
                return;
            pc.Job = job;
            pc.Level1 = pc.Level;
            pc.Level = 1;
        ...
        /// </summary>
        /// <param name="pc"></param>
        protected void OpenAncientArk(ActorPC pc)
        {
            MapClient client = GetMapClient(pc);

            SagaMap.Packets.Server.SSMG_AARCH_QUEST p1 = new SagaMap.Packets.Server.SSMG_AARCH_QUEST();
            p1.AA_Clear = pc.AncientArk_Clear;
            p1.Rebirth = pc.Rebirth;
            p1.Six_boss = false;
            MapClient.FromActorPC(pc).netIO.SendPacket(p1);
            SagaMap.Packets.Server.SSMG_AARCH_QUEST_OPEN p2 = new SagaMap.Packets.Server.SSMG_AARCH_QUEST_OPEN();
            MapClient.FromActorPC(pc).netIO.SendPacket(p2);

            client.AncientArk = true;
            bool blocked = ClientManager.Blocked;
            if (blocked)
                ClientManager.LeaveCriticalArea();
            while (client.AncientArk)
            {
                System.Threading.Thread.Sleep(500);
            }
            if (blocked)
