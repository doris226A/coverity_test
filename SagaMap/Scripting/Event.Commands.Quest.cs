...
namespace SagaMap.Scripting
{
    public abstract partial class Event
    {
        ...
        public void SendQuestInfo(ActorPC pc)
        {
        ...
        protected void HandleQuest(ActorPC pc, uint groupID)
        {
            ...
            if (pc.Quest != null)
            {
                ...
                if (pc.Quest.Status == QuestStatus.COMPLETED)
                {
                    ...
                    pc.Gold += (int)(pc.Quest.Detail.Gold * Configuration.Instance.CalcQuestGoldRateForPC(pc));
                    pc.CP += (uint)(pc.Quest.Detail.CP);
                    client.EP_Add(pc.Quest.Detail.EP);//ECOKEY 新增EP獎勵
                   // Logger.ShowInfo("pc.Quest.Detail.EP  " + pc.Quest.Detail.EP);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (quest != null)
                    {
                        //ECOKEY 更改成客戶端的任務獎勵(EP改)
                        if (SagaDB.Quests.QuestRewardFactory.Instance.Items.ContainsKey(quest.ID))
                        {
                            QuestRewardInfo q = SagaDB.Quests.QuestRewardFactory.Instance.Items[quest.ID];
                            quest.Detail.EXP = q.EXP;
                            quest.Detail.JEXP = q.JEXP;
                            quest.Detail.Gold = q.Gold;
                            quest.Detail.EP = q.EP;
                        }
