...
namespace SagaMap.Network.Client
{
    public partial class MapClient
    {
        ...
        public void OnNPCChoiceBuy(Packets.Client.CSMG_NPC_CHOICE_BUY p)
        {
            ...
            uint gold = (uint)Character.Gold;

            foreach (uint goodID in goods)
            {
                if (!KujiListFactory.Instance.ItemChoiceList.ContainsKey(goodID))
                {
                    SendSystemMessage("該商品不存在。");
                    return;
                }
            }
            ...
            foreach (uint goodID in goods)
            {
                uint price = KujiListFactory.Instance.ItemChoiceList[goodID];
                ushort count = counts[Array.IndexOf(goods, goodID)];
            ...
            {
                uint goodID = goods[i];
                ushort count = counts[i];

                if (KujiListFactory.Instance.ItemChoiceList.ContainsKey(goodID))
                {
                    Item item = ItemFactory.Instance.GetItem(goodID);
                    item.Stack = count;

                    if (item.BaseData.itemType == ItemType.EFFECT)
                    {
                        SendSystemMessage("特效物品『" + item.BaseData.name + "』無法購買。");
                        continue;
                    }
                    ...
                    {
                        gold -= price;
                        Logger.LogItemGet(Logger.EventType.ItemNPCGet, $"{Character.Name}({Character.CharID})", $"{item.BaseData.name}({item.ItemID})", $"ChoiceShopBuy Count:{item.Stack}", false);
                        AddItem(item, false);
                    }
                }
            }
            ...
            if (!vshopClosed)
            {
                ...
                if (this.Character.VShopPoints >= neededPoints)
                {
                    ...
                    for (k = 0; k < items.Length; k++)
                    {
                        ...
                        //新增購買紀錄
                        Logger logger = new Logger("玩家商城購買.txt");
                        string log = "\r\n玩家：" + this.Character.Name + "(" + this.Character.CharID + "),購買了" + item.BaseData.name + "(" + item.ItemID + ")," + item.Stack + "個";
                        logger.WriteLog(log);
        ...
        public void OnNCShopBuy(Packets.Client.CSMG_NCSHOP_BUY p)
        {
            ...
            if (this.Character.NCShopPoints >= neededPoints)
            {
                this.Character.UsedNCShopPoints += neededPoints;
                this.Character.NCShopPoints -= neededPoints;
                for (k = 0; k < items.Length; k++)
                {
                    if (counts[k] <= 0)
                        continue;
      ...
      public void HandleNCShopBuy(Packets.Client.CSMG_NCSHOP_BUY p)
      {
            ...
            if (p.Unknown != 0)
            {
                ...
                if (item != null || p.ItemUseCount == 0)
                {
                    if (item != null)
                    {
                        ...
                        {
                            //DeleteItem(item.Slot, (ushort)p.ItemUseCount, true); 轉職時系統會出現：失去Item/記憶之沙X個
                        }
                        else
                            return;
                    }
        ...
        public void OnNPCShopBuy(Packets.Client.CSMG_NPC_SHOP_BUY p)
        {
            uint[] goods = p.Goods;
            uint[] counts = p.Counts;
            var IsCreditCardDeal = p.IsCreditCardDeal;
            if (Character.HP == 0) return;
            if (this.currentShop != null && goods.Length > 0)
            {
                ...
                switch (this.currentShop.ShopType)
                {
                    case ShopType.None:
                        if (IsCreditCardDeal)
                            gold = (long)this.Character.Account.Bank;
                        else
                ...
                for (int i = 0; i < goods.Length; i++)
                {
                    if (this.currentShop.Goods.Contains(goods[i]))
                    {
                        ...
                        long price = (long)((float)item.BaseData.price * (float)(this.currentShop.SellRate / 100.0f) * (float)(buyrate / 100.0f) * (float)((float)this.Character.Status.buy_rate_iris / 100.0f));
                        ...
                        if (this.currentShop.ShopType == ShopType.None && IsCreditCardDeal)
                            price = (long)Math.Ceiling((float)((float)price * 1.1f));
                ...
                if (this.currentEvent != null)
                {
                    ...
                    for (int i = 0; i < goods.Length; i++)
                    {
                        if (this.currentEvent.Goods.Contains(goods[i]))
                        {
                            Item item = ItemFactory.Instance.GetItem(goods[i]);
                            item.Stack = (ushort)counts[i];
                            long price = (long)(item.BaseData.price * ((float)(this.Character.Status.buy_rate) / 100.0f) * (float)((float)this.Character.Status.buy_rate_iris / 100.0f));
                            if (price == 0) price = 1;
                            price = price * item.Stack;
                            if (gold >= price)
                            {
        ...
        public void OnNPCShopSell(Packets.Client.CSMG_NPC_SHOP_SELL p)
        {
            ...
            if (this.currentShop != null)
            {
                ...
                for (int i = 0; i < goods.Length; i++)
                {
                    ...
                    if (itemDroped.BaseData.noSoldToNPC && TradeConditionFactory.Instance.Items[itemDroped.ItemID].GMLevelToIgnoreCondition > this.Character.Account.GMLevel)
                    {
                        this.SendSystemMessage("无法出售道具[" + itemDroped.BaseData.name + "]" + itemDroped.ItemID);
                        continue;
                    }
                    ...
                    long price = (long)((float)itemDroped.BaseData.price * (float)counts[i] * (float)(this.currentShop.BuyRate / 100.0f) * (float)((float)this.Character.Status.sell_rate / 100.0f) * (float)((float)this.Character.Status.sell_rate_iris / 100.0f));
                    total += price;
                }
            ...
            else
            {
                if (this.currentEvent != null)
                {
                    ...
                    for (int i = 0; i < goods.Length; i++)
                    {
                        ...
                        long price = (long)((float)itemDroped.BaseData.price * (float)counts[i] * ((float)this.Character.Status.sell_rate / 100.0f) * (float)((float)this.Character.Status.sell_rate_iris / 100.0f));
        ...
        {
            if (this.Character.Account.GMLevel > 90)
                this.SendSystemMessage("触发ID:" + EventID.ToString());

            if (this.Character.AStr["language"] == "ENGLISH")//ECOKEY 新增英文腳本
            {
                if (ScriptENManager.Instance.Events.ContainsKey(EventID))
                {
        ...
        void RunScript()
        {
            ...
            try
            {
                if (this.Character.AStr["language"] == "ENGLISH")//ECOKEY 新增英文腳本
                {
                    evnt = ScriptENManager.Instance.Events[currentEventID];
                }
                else
                {
                    ...
                    if (this.Character.Quest.Detail.NPCDestination == evnt.EventID)
                    {
                        if (this.Character.Quest.CurrentCount2 == 0 && this.Character.Quest.Status == SagaDB.Quests.QuestStatus.OPEN)
                        {
                            ...
                            if (this.Character.Quest.CurrentCount3 == 0)
                            {
                                ...
                                SendQuestStatus();
                                runscript = false;
                                //ECOKEY 為了東門新增（不要了，可刪）
                                /*if (currentEventID == 11001447)
                                {
                                    runscript = true;
                                }*/
                            }
