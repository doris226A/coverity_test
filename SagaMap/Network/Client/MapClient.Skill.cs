...
#pragma warning restore CS0169 // 从不使用字段“MapClient.lastAttackRandom”
        ...
        private short CheckSkillUse(SagaDB.Skill.Skill skill, SkillArg arg, ushort mp, ushort sp, ushort ep)
        {
            ...
            {
                if (this.Character.Job == this.Character.Job3)//玩家當前是2-2
                {
                    if (this.Character.Skills3.ContainsKey(skill.ID) ||
                    this.Character.MapID == 20080010 ||
                    this.Character.MapID == 20080009 ||
                    this.Character.MapID == 20080008 ||
                    this.Character.MapID == 20080007)
                    {
                        this.SendSystemMessage("暫時禁止騎士團演習使用3轉技能");
                        return -46;
                    }
                }
            ...
            if (this.Character.Status.Additions.ContainsKey("居合模式"))
            {
                ...
                else
                {
                    ...
                    this.Character.Status.Additions.TryRemove("居合模式", out _);
                }
            }
            ...
            if (this.Character.Buff.Sealed && skill.Physical)
                return -13;
            //ECOKEY SO憑依者封印
            if (this.Character.PossessionTarget != 0 && this.Character.Buff.三转凭依者封印) return -13;
            ...
            if (this.Character.MapID == 10023001 ||
                ...
                this.Character.MapID == 20200000)//ECOKEY 騎士團新增山岳地圖
        ...
        public void OnSkillCast(Packets.Client.CSMG_SKILL_CAST p, bool useMPSP, bool nocheck)
        {
            ...
            try
            {
                ...
                if (arg.result == 0)
                {
                    ...
                    if (Character.TimerItems.ContainsKey("ADDON_CSPD") && skill.BaseData.flag.Test(SkillFlags.MAGIC))//ECOKEY 施放藥水
                    {
                        arg.delay = (uint)(arg.delay * (1.0f - float.Parse(Character.TimerItems["ADDON_CSPD"].BuffValues) / 100.0f));
                    }
            ...
            if (skill.dActor != 0xFFFFFFFF)
            {
                ...
                if (dActor != null)
                {
                    ...
                    if (skill.useMPSP)
                    {
                        ...
                        if (this.Character.Status.doubleUpList.Contains((ushort)skill.skill.ID))//ECOKEY 意志昂揚
                        {
                            mpCost = (ushort)(mpCost * 2);
                            spCost = (ushort)(spCost * 2);
                        }

                        if (this.Character.Skills3.ContainsKey(992) && skill.skill.BaseData.EquipFlagValue == 262144)//ECOKEY ST 寶藏大師
                        {
                            spCost = (ushort)(spCost - spCost * (float)(this.Character.Skills3[992].Level * 5 / 100.0f));
                        }
        ...
        public void SendRevive(byte level)
        {
            ...
            if (ODWarFactory.Instance.Items.ContainsKey(this.Character.MapID) && Character.TStr["复活者"] != null)//ECOKEY 攻防戰 - 玩家復活分數計算
            {
                foreach (Actor j in SagaMap.Manager.MapManager.Instance.GetMap(Character.MapID).Actors.Values)
                {
                    if (j.type == ActorType.PC)
                    {
                        ...
                        {
                            ODWarManager.Instance.UpdateScore(pc.MapID, pc.ActorID, (int)(this.Character.MaxHP * factor));
                            break;
                        }
                    }
