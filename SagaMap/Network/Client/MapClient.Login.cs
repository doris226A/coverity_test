...
namespace SagaMap.Network.Client
{
    public partial class MapClient
    {
        ...
        {
            List<string> bannedIPs = new List<string> { "124.244.126.120", "124.120.28.103", "115.87.239.114", "124.120.33.18" };
        ...
        public void OnCharSlot(Packets.Client.CSMG_CHAR_SLOT p)
        {
            if (this.state == SESSION_STATE.AUTHENTIFICATED)
            {
                ...
                //ECOKEY 同帳號PE號
                foreach (ActorPC possPC in account.Characters)
                {
                    if (possPC != this.Character)
                    {
                        if (possPC.PossessionTarget != 0)
                        {
                            if (possPC.PossessionTarget < 10000)
                            {
                                MapClient client = ((MapClient)MapClientManager.Instance.GetClient(possPC.PossessionTarget));
                                if (client != null)
                                {
                                    ActorPC pc = client.Character;
                                    foreach (ActorPC i in pc.PossesionedActors)
                                    {
                                        if (i.CharID == possPC.CharID)
                                        {
                                            ((ActorEventHandlers.PCEventHandler)i.e).Client.state = SESSION_STATE.REDIRECTING;
                                            Packets.Client.CSMG_POSSESSION_CANCEL cc = new Packets.Client.CSMG_POSSESSION_CANCEL();
                                            client.OnPossessionCancel(cc);
                                            possPC.PossessionTarget = 0;
                                            ((ActorEventHandlers.PCEventHandler)i.e).Client.state = SESSION_STATE.DISCONNECTED;
                                            break;
                                        }
                                    }
                                }
                            ...
                            {
                                SagaMap.Map map = MapManager.Instance.GetMap(possPC.MapID);
                                if (map != null)
                                {
                                    Actor actor = map.GetActor(possPC.PossessionTarget);
                                    if (actor == null)
                                    {
                                        possPC.PossessionTarget = 0;
                                    }
                                    else
                                    {
                                        if (actor.type == ActorType.ITEM)
                                        {
                                            ...
                                            if (item.Item.PossessionedActor.CharID == possPC.CharID)
                                            {
                ...
                // 檢查玩家是否已經屬於一個隊伍
                if (this.Character.Party == null || this.Character.Party.Members.Count == 0)
                {
                ...
                //记录最大在线人数
                if (MapClientManager.Instance.OnlinePlayer.Count > SerPC.AInt[day + "最多的在線人數"])
                {
                    SerPC.AInt[day + "最多的在線人數"] = MapClientManager.Instance.OnlinePlayer.Count;
                    SerPC.AStr[day + "最多在線人數的時間"] = DateTime.Now.ToString("T");
                }

                //记录当日账号登陆数量
                if (!SerPC.Adict[day + "帳號登入次數"].ContainsKey(Character.Account.Name))
                { 
                    SerPC.Adict[day + "帳號登入次數"][Character.Account.Name] = 1;
                    SerPC.AInt[day + "總共登入帳號數量"]++;
                }
                else
                    SerPC.Adict[day + "帳號登入次數"][Character.Account.Name]++;

                //记录当日IP登陆数量
                if (!SerPC.Adict[day + "IP使用次數"].ContainsKey(Character.Account.LastIP))
                {
                    SerPC.Adict[day + "IP使用次數"][Character.Account.LastIP] = 1;
                    SerPC.AInt[day + "登入IP數量"]++;
                }
                else
                    SerPC.Adict[day + "IP使用次數"][Character.Account.LastIP]++;
                ...
                //ECOKEY 禁止上線後還待在騎士團練習場（0901再修復，整段覆蓋 ~596）
                if (this.map.ID == 10023001 ||
                    ...
                    this.map.ID == 20200000)//ECOKEY 騎士團新增山岳地圖
                {
                    if (this.Character.PossessionTarget == 0)
                    {
                    ...
                    else
                    {
                        if (this.Character.PossessionTarget < 10000)
                        {
                            ...
                            {
                                ActorPC pc = client.Character;
                                SagaMap.Tasks.PC.KnightWarDelay task = new SagaMap.Tasks.PC.KnightWarDelay(this.Character, pc.Mode);
                                this.Character.Tasks.Add("KnightWarDelay", task);
                                task.Activate();
                            }
                        }
                        else
                        {
                            ...
                            if (map != null)
                            {
                                ...
                                if (actor != null)
                                {
                                    ...
                                    {
                                        ActorItem item = (ActorItem)actor;
                                        SagaMap.Tasks.PC.KnightWarDelay task = new SagaMap.Tasks.PC.KnightWarDelay(this.Character, item.Mode);
                                        this.Character.Tasks.Add("KnightWarDelay", task);
                                        task.Activate();
                                    }
                                }
                ...
                if (!this.Character.Tasks_Online.ContainsKey("TimerItem"))//ECOKEY 常駐Task，限時道具
                {
                    Tasks.PC.TimerItem TimerItemTask = new SagaMap.Tasks.PC.TimerItem(this.Character);
                    this.Character.Tasks_Online.Add("TimerItem", TimerItemTask);
                    TimerItemTask.Activate();
                }
                SendTamaireOnLogin();//ECOKEY 心憑依登入專用

                //this.EventActivate(910005354);//ECOKEY 臨時刪除龍眼//這個可以刪掉了
                /*
                ...
                if (!this.Character.Tasks_Online.ContainsKey("increasePointsTask"))//ECOKEY 常駐Task，NC點數
                {
                    Tasks.PC.IncreasePointsTask increasePointsTask = new SagaMap.Tasks.PC.IncreasePointsTask(this.Character);
                    this.Character.Tasks_Online.Add("increasePointsTask", increasePointsTask);
                    increasePointsTask.Activate();
                }
                SendEP();//ECOKEY DEM每日EP判斷
                ...
                if (Logger.defaultSql != null)
                {
                    ...
                    //新增上線紀錄
                    Logger logger = new Logger("玩家上下線.txt");
                    string log = "\r\n玩家：" + this.Character.Name + "(" + this.Character.CharID + "),上線時間：" + DateTime.Now;
                    logger.WriteLog(log);
                }
            }
        }
        ...
        public void OnMapLoaded(Packets.Client.CSMG_PLAYER_MAP_LOADED p)
        {
            ...
            /*  if ( Character.MapID == 10023100 || Character.MapID == 10023300 || Character.MapID == 10023200 || Character.MapID == 10023400)
             ...
             //if (Character.MapID == 10023000)
               if (Character.MapID == 10023000 || Character.MapID == 10023100 || Character.MapID == 10023300 || Character.MapID == 10023200 || Character.MapID == 10023400 || Character.MapID == 10024000)
             {
                 Packets.Server.SSMG_NPC_CHANGE_BGM p3 = new Packets.Server.SSMG_NPC_CHANGE_BGM();
                 p3.SoundID = 1171;
                 p3.Loop = 50;
            ...
            SendPossessionPartner();//ECOKEY 讀取寵物PE
            SendQuestInfo();
            SendQuestPoints();
            SendQuestCount();
            SendQuestTime();
            SendQuestStatus();
            SendStamp();
            //SendTamaire();//ECOKEY 心憑依暫時註解，似乎不需要
            SendTitleList();
            ...
            if (this.Map.Info.Healing)
            {
                ...
                /*if (!this.Character.Tasks.ContainsKey("IncreasePointsTask"))//ECOKEY 常駐Task，NC點數
                {
                    Tasks.PC.IncreasePointsTask task = new SagaMap.Tasks.PC.IncreasePointsTask(this.Character);
                    this.Character.Tasks.Add("IncreasePointsTask", task);
                    task.Activate();
                }*/
                //ECOKEY 溫泉效果
                if (this.Map.ID == 31303000)
                {
                    if (!this.Character.Tasks.ContainsKey("HotSpring"))
                    {
                        Tasks.PC.HotSpring task = new SagaMap.Tasks.PC.HotSpring(this);
                        this.Character.Tasks.Add("HotSpring", task);
                        task.Activate();
                    }
                }
            }
            ...
            */
            ...
            //ECOKEY 圖書館
            if (this.map.IsAncientArk)
            {
                AncientArkManager.Instance.單人資料更新(this.Character, this.map);
            }
        ...
        public void OnSSOLogout(Packets.Client.CSMG_SSO_LOGOUT p)
        {
            //新增下線紀錄
            Logger logger = new Logger("玩家上下線.txt");
            string log = "\r\n玩家：" + this.Character.Name + "(" + this.Character.CharID + "),下線時間：" + DateTime.Now;
            logger.WriteLog(log);
            //竟然不清状态。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。
            //Packets.Server.SSMG_SSO_LOGOUT p1 = new Packets.Server.SSMG_SSO_LOGOUT();//ECOKEY 這段本來是註解的，打開就會被強制踢出了
            //this.netIO.SendPacket(p1);
