...
namespace SagaMap.Network.Client
{
    public partial class MapClient
    {
        ...
        public void OnRecruitJoin(Packets.Client.CSMG_COMMUNITY_RECRUIT_JOIN p)
        {
            ...
            //ECOKEY 招募組隊申請隊長
            if (target != null && target.Character.Party != null && target.Character.Party.Leader != target.Character)
            {
                target = MapClientManager.Instance.FindClient(target.Character.Party.Leader);
                if (target == null) result = -4;
            }
            Packets.Server.SSMG_COMMUNITY_RECRUIT_JOIN_RES p1 = new SagaMap.Packets.Server.SSMG_COMMUNITY_RECRUIT_JOIN_RES();
            p1.Result = result;
            p1.CharID = p.CharID;
