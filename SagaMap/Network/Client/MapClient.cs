    ...
    public partial class MapClient : SagaLib.Client
    {
        ...
        void SendHack()
        {
            ...
            if (hackCount > 10)
            {
                ...
                {
                    i.SendAnnounce("伺服器偵測到" + this.Character.Name +"正在使用輔助程式或外掛程式！");
                }
                //this.netIO.Disconnect();
            }
            else if (hackCount > 2)
            {
                this.SendSystemMessage("就跟你講了別用輔助程式或外掛程式。");
            }
            /*try
            {
                if ((this.hackStamp - DateTime.Now).TotalMinutes >= 10)
        ...
        public override void OnDisconnect()
        {
            ...
            try
            {
                ...
                this.chara.ClearTaskOnline();//ECOKEY 常駐Task，清除
                    ...
                    else
                    {
                        if (i != this.Character)
                        {
                            ...
                            if (i.Online)
                            {
                                ...
                                this.Map.SendEventToAllActorsWhoCanSeeActor(Map.EVENT_TYPE.POSSESSION, arg, i, true);
                                i.PossessionPosition = PossessionPosition.NONE;
                            }
                            else
                            {
                ...
                //ECOKEY 儲存寵物憑依資料
                foreach (ActorPartner i in Character.PossesionedPartner.Values)
                {
                    MapServer.charDB.SavePartner(i);
                }
                ...
                if (golemLogout && this.chara.PossessionTarget == 0)
                {
                    ...
                    if ((long)this.Character.Golem.BuyLimit > this.Character.Gold)
                ...
                //ECOKEY 換個位置，先存檔再刪Actor，看能不能解決寵物消失的問題
                ...
                else
                {
                    ...
                    uint potarget = this.Character.PossessionTarget;
                    Actor targetactor = Map.GetActor(potarget);
                    if (targetactor is ActorPC)
                    {
                        var potargetpc = targetactor as ActorPC;
                        potargetpc.PossesionedActors.Remove(this.Character);
                    }
                }
