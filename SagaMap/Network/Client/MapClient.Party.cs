   ...
   || client.Character.Mode == PlayerMode.KNIGHT_ROCK || client.Character.Mode == PlayerMode.KNIGHT_SOUTH || client.Character.Mode == PlayerMode.KNIGHT_WEST)
    ...
    || this.Character.Mode == PlayerMode.KNIGHT_ROCK || this.Character.Mode == PlayerMode.KNIGHT_SOUTH || this.Character.Mode == PlayerMode.KNIGHT_WEST)
            ...
            else
            {
                ...
                團結力量被動判定(this.Character, true);
                團結力量被動判定(client.Character, true);
            }
        }
        ...
        private int CheckPartyInvite(MapClient client)
        {
            if (client == null)
                return -2;  //プレイヤーが存在しません
                            //ECOKEY 隊伍拒絕設定
            if (client.scriptThread != null || !client.Character.canParty)
        ...
        void 團結力量被動判定(ActorPC pc, bool change)
        {
            #region "團結力量"
            //ECOKEY 團結力量
            if (pc == null || pc.Status == null || pc.Status.Additions == null)
                return;

            if (pc.JobBasic == PC_JOB.SWORDMAN || pc.JobBasic == PC_JOB.FENCER || pc.JobBasic == PC_JOB.SCOUT || pc.JobBasic == PC_JOB.ARCHER)
            {
                Dictionary<uint, SagaDB.Skill.Skill> skillsToCheck = new Dictionary<uint, SagaDB.Skill.Skill>();
                if (pc.Skills2.ContainsKey(978))
                    skillsToCheck = pc.Skills2;
                else if (pc.SkillsReserve.ContainsKey(978))
                    skillsToCheck = pc.SkillsReserve;
                else if (pc.Skills2_1.ContainsKey(978))
                    skillsToCheck = pc.Skills2_1;
                if (skillsToCheck.TryGetValue(978, out var skill))
                {
                    if (change)
                    {
                        ...
                        SagaMap.Skill.SkillHandler.Instance.skillHandlers[978].Proc(pc, pc, arg, skill.Level);
                    }
                    else
                    {
                        if (pc.Status.Additions.ContainsKey("AtkUpByPt"))
                        {
                            pc.Status.Additions["AtkUpByPt"].AdditionEnd();
                            pc.Status.Additions.TryRemove("AtkUpByPt", out _);
                        }
                    }
                }
            }
              ...
              if (pc.JobBasic == PC_JOB.SWORDMAN || pc.JobBasic == PC_JOB.FENCER || pc.JobBasic == PC_JOB.SCOUT || pc.JobBasic == PC_JOB.ARCHER)
              {
                  if (!pc.Rebirth || pc.Job != pc.Job3)
                  {
                      if (pc.Skills2.ContainsKey(978))
                      {
                          ...
                          {
                              SkillArg arg = new SkillArg();
                              arg.skill = pc.Skills2[978];
                              SagaMap.Skill.SkillHandler.Instance.skillHandlers[978].Proc(pc, pc, arg, pc.Skills2[978].Level);
                          }
                          else
                          {
                              pc.Status.Additions["AtkUpByPt"].AdditionEnd();
                              pc.Status.Additions.Remove("AtkUpByPt");
                          }

                      }
                      if (pc.SkillsReserve.ContainsKey(978))
                      {
                          if (change)
                          {
                              SkillArg arg = new SkillArg();
                              arg.skill = pc.SkillsReserve[978];
                              SagaMap.Skill.SkillHandler.Instance.skillHandlers[978].Proc(pc, pc, arg, pc.SkillsReserve[978].Level);
                          }
                          else
                          {
                              if (pc.Status.Additions.ContainsKey("AtkUpByPt"))
                  ...
                  {
                      if (pc.Skills2_1.ContainsKey(978))
                      {
                          if (change)
                          {
                              SkillArg arg = new SkillArg();
                              arg.skill = pc.Skills2_1[978];
                              SagaMap.Skill.SkillHandler.Instance.skillHandlers[978].Proc(pc, pc, arg, pc.Skills2_1[978].Level);
                          }
                          else
                          {
                              if (pc.Status.Additions.ContainsKey("AtkUpByPt"))
