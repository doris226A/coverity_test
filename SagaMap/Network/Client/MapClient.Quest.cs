        ...
        public void QuestMobKilled(ActorMob mob, bool party)
        {
            if (this.Character.Quest != null)
            {
                if (this.Character.Quest.QuestType == SagaDB.Quests.QuestType.HUNT)
                {
                    ...
                    //假設怪物地圖=任務地圖 假設角色地圖=任務地圖
                    if (mob.MapID == this.Character.Quest.Detail.MapID1 || this.Character.MapID == this.Character.Quest.Detail.MapID1 ||
                          mob.MapID == this.Character.Quest.Detail.MapID2 || this.Character.MapID == this.Character.Quest.Detail.MapID2 ||
                          mob.MapID == this.Character.Quest.Detail.MapID3 || this.Character.MapID == this.Character.Quest.Detail.MapID3||
                           (this.Character.Quest.Detail.MapID1 == 60000000 && this.map.IsDungeon))


                        if (mob.MapID != this.Character.MapID)
                           return;

                    if (mob.MapID == this.Character.Quest.Detail.MapID1 || mob.MapID == this.Character.Quest.Detail.MapID2 || mob.MapID == this.Character.Quest.Detail.MapID3 ||
                       (this.Character.Quest.Detail.MapID1 == 0 && this.Character.Quest.Detail.MapID2 == 0 && this.Character.Quest.Detail.MapID3 == 0) || //千姬任務（不指定地圖任務）
                       this.Character.Quest.Detail.MapID1 == 60000000 && this.map.IsDungeon)
                    //  if (mob.MapID == this.Character.Quest.Detail.MapID1 || mob.MapID == this.Character.Quest.Detail.MapID2 || mob.MapID == this.Character.Quest.Detail.MapID3 ||
                    //      this.Character.Quest.Detail.MapID1 == 60000000 && this.map.IsDungeon)
        ...
        public void EventMobKilled(ActorMob mob)
        {
            ...
            foreach (KeyValuePair<uint, ActorPC.KillInfo> i in this.Character.KillList)
            {
                if (!i.Value.isFinish)
                {
                    ...
                    {
                        i.Value.Count++;
                        this.SendSystemMessage("擊殺任務：已擊殺 " + mob.BaseData.name + " (" + i.Value.Count.ToString() + "/" + i.Value.TotalCount.ToString() + ")");
                        if (i.Value.Count == i.Value.TotalCount)
                            this.SendSystemMessage("擊殺任務：擊殺 " + mob.BaseData.name + " 已完成！");
                    }
                    if (i.Value.Count >= i.Value.TotalCount)
