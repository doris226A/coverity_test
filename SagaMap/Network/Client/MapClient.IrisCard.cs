...
namespace SagaMap.Network.Client
{
    public partial class MapClient
    {
        ...
        uint irisCardItem = 0;
        public bool irisCardNoMoney = false;//ECOKEY 技能專用，不用G
        public bool irisCardArms = false;//ECOKEY 技能專用，提高除了弓、杖以外武器機率
        public bool irisCardBow = false;//ECOKEY 技能專用，提高弓機率
        public bool irisCardStaff = false;//ECOKEY 技能專用，提高杖機率
        public bool irisCardNeck = false;//ECOKEY 技能專用，提高項鍊機率
        public bool irisCardArmor = false;//ECOKEY 技能專用，提高防具機率
            ...
            if (CountItem(itemID) > 0)
            {
                if (IrisGachaFactory.Instance.IrisGacha.ContainsKey(key))
                {
                    ...
                    for (int i = 0; i < gacha.Count; i++)
                    {
                        ...
                        if (lottery < (drawrate != null ? drawrate.SuperRatityRate : 0.5))
                            rank = 4; // Super Rare: 0.05%
                        else if (lottery < (drawrate != null ? drawrate.RatityRate : 90))
                            rank = 3; // Rare: 8.9%
                        else if (lottery < (drawrate != null ? drawrate.UnCommonRate : 200))
                            rank = 2; // Uncommon: 11%
                        else
                            rank = 1; // Common: 80%
            ...
            if (CountItem(cardID) > 0)
            {
                if (IrisCardFactory.Instance.Items.ContainsKey(cardID))
                {
                    ...
                    if (card.NextCard != 0)
                    {
                        ...
                        int[] rates = new int[4] { 90, 70, 40, 20 };
                        int[] counts = new int[4] { 10, 2, 2, 2 };
                        var SupportItem = p.SupportItem;
                        var ProtectItem = p.ProtectItem;


                        //ECOKEY 曈修正，新增SendSystemMessage提示
                        // 迪仔
                        /*if (!(SupportItem == 10087100 || SupportItem == 10087101 || //SupportItem == 16003600 ||
                              SupportItem == 16003700 || SupportItem == 16003701 || SupportItem == 16003702 ||
                              SupportItem == 16003703) || CountItem(SupportItem) == 0)
                        {
                        ...
                        if (SupportItem != 0 && CountItem(SupportItem) == 0)
                        {
                            SendSystemMessage("輔助道具不存在");
                            Packets.Server.SSMG_IRIS_CARD_ASSEMBLE_RESULT p1 = new SagaMap.Packets.Server.SSMG_IRIS_CARD_ASSEMBLE_RESULT();
                            p1.Result = SagaMap.Packets.Server.SSMG_IRIS_CARD_ASSEMBLE_RESULT.Results.FAILED;
                            this.netIO.SendPacket(p1);
                            irisCardAssemble = false;
                        ...
                        // 检查保护道具是否有效
                        if (ProtectItem != 0 && CountItem(ProtectItem) == 0)
                        {
                            SendSystemMessage("輔助道具不存在");
                            Packets.Server.SSMG_IRIS_CARD_ASSEMBLE_RESULT p1 = new SagaMap.Packets.Server.SSMG_IRIS_CARD_ASSEMBLE_RESULT();
                            p1.Result = SagaMap.Packets.Server.SSMG_IRIS_CARD_ASSEMBLE_RESULT.Results.FAILED;
                        ...
                        if (this.chara.Skills.ContainsKey(2009)) rate += 3;//ECOKEY 精緻道具技能增加成功率
                        ...
                        if (CountItem(cardID) >= count)
                        {
                            if (this.chara.Gold >= 5000)
                            {
                                ...
                                Logger.LogItemLost(Logger.EventType.ItemNPCLost, this.chara.Name + "(" + this.chara.CharID + ")", card.Name + "(" + cardID + ")",
                                    string.Format("IrisCardAssemble supportitem:{0},count:{1},protectitem:{2}", SupportItem, count, ProtectItem), true);
        ...
        protected void ItemAddSlot(ActorPC pc)
        {
            ...
            //スロット追加の奧義
            if (CountItem(16022400) > 0)
                items.Add(GetItem(16022400)[0].Slot);
        ...
        public void OnIrisCardRemove(CSMG_IRIS_CARD_REMOVE p)
        {
            ...
            if (item != null)
            {
                if (item.PossessionedActor != null || item.PossessionOwner != null)
                {
                    ...
                    p1.Result = SSMG_IRIS_CARD_REMOVE_RESULT.Results.CANNOT_REMOVE_NOW;
                    this.netIO.SendPacket(p1);
                    return;
                }
        ...
        //ECOKEY 新增
        public void OnIrisCardRemoveSlot(CSMG_IRIS_CARD_REMOVE_SLOT p)
        {
            Item item = this.chara.Inventory.GetItem(irisCardItem);
            if (item != null)
            {
        ...
        //ECOKEY 新增
        public void OnIrisCardClear(CSMG_IRIS_CARD_CLEAR p)
        {
            Item item = this.chara.Inventory.GetItem(irisCardItem);
            if (item != null)
            {
                ...
                if (!item.Locked)
                {
                    ...
                    p1.Result = SagaMap.Packets.Server.SSMG_IRIS_CARD_REMOVE_RESULT.Results.OK;
                    this.netIO.SendPacket(p1);
                    foreach (IrisCard i in item.Cards)
                    {
                        IrisCard card = i;
                        AddItem(ItemFactory.Instance.GetItem(card.ID), true);
        ...
        public void OnIrisCardInsert(CSMG_IRIS_CARD_INSERT p)
        {
            ...
            if (item != null)
            {
                if (item.PossessionedActor != null || item.PossessionOwner != null)
                {
                    ...
                    p1.Result = SSMG_IRIS_CARD_INSERT_RESULT.Results.CANNOT_SET_NOW;
                    this.netIO.SendPacket(p1);
                    return;
                }
        ...
        public void OnIrisCardOpen(CSMG_IRIS_CARD_OPEN p)
        {
            ...
            if (item != null)
            {
                if (item.PossessionedActor != null || item.PossessionOwner != null)
                {
                    ...
                    p1.Result = SagaMap.Packets.Server.SSMG_IRIS_CARD_OPEN_RESULT.Results.CANNOT_SET_NOW;
                    this.netIO.SendPacket(p1);
                }
                    ...
                    if (CountItem(material) > 0)
                    {
                        if (this.chara.Gold > gold)
                        {
                            if ((!item.EquipSlot.Contains(EnumEquipSlot.CHEST_ACCE) && item.CurrentSlot < 10) || (item.EquipSlot.Contains(EnumEquipSlot.CHEST_ACCE) && item.CurrentSlot < 5))
                            {
                                ...
                                int[] rates = { 80, 70, 60, 50, 40, 30, 20, 15, 10, 5 };
                                int[] rates_CHEST_ACCE = { 80, 70, 60, 50, 40 };
                                int baseRate = 0;
                                ...
                                if (!item.EquipSlot.Contains(EnumEquipSlot.CHEST_ACCE))
                                    baseRate = rates[item.CurrentSlot];
                                else
                                    baseRate = rates_CHEST_ACCE[item.CurrentSlot];

                                Logger.LogItemLost(Logger.EventType.ItemNPCLost, this.chara.Name + "(" + this.chara.CharID + ")", item.Name + "(" + item.ItemID + ")",
                                    string.Format("IrisSlotTake supportitem:{0},item.CurrentSlot:{1},protectitem:{2}", supportitem, item.CurrentSlot, protectitem), true);

                                if (baseRate < 0)
                                    baseRate = 5;

                                //ECOKEY 精緻道具技能增加成功率
                                if (irisCardNoMoney) baseRate += 2;
                                if (item.EquipSlot.Contains(EnumEquipSlot.CHEST_ACCE) && irisCardNeck)
                                {
                                    baseRate += 2;
                                }
                                else if (item.EquipSlot.Contains(EnumEquipSlot.UPPER_BODY) && irisCardArmor)
                                {
                                    baseRate += 2;
                                }
                                else if (item.BaseData.itemType == ItemType.STAFF && irisCardStaff)
                                {
                                    baseRate += 2;
                                }
                                else if (item.BaseData.itemType == ItemType.BOW && irisCardBow)
                                {
                                    baseRate += 2;
                                }
                                else if (item.BaseData.itemType != ItemType.STAFF && item.BaseData.itemType != ItemType.BOW && item.EquipSlot.Contains(EnumEquipSlot.RIGHT_HAND) && irisCardArms)
                                {
                                    baseRate += 2;
                                }
                                ...
                                else if (supportitem == 16022400 && item.CurrentSlot <= 5)//補スロット追加の奧義
                                {
                                    DeleteItemID(supportitem, 1, true);
                                    baseRate += 100;
                                }
                                ...
                                else if (protectitem != 0)
                                {
                                    ...
                                    this.SendSystemMessage("裝備打洞失敗！失去一本防爆書。");
                                    Packets.Server.SSMG_IRIS_ADD_SLOT_RESULT p1 = new SagaMap.Packets.Server.SSMG_IRIS_ADD_SLOT_RESULT();
