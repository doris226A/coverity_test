...
namespace SagaMap.Mob
{
    public partial class MobAI
    {
        ...
        public void CastSkill(uint skillID, byte lv, uint target, short x, short y)
        {
            ...
            //ECOKEY CA的封印
            if (actor.Buff.MagicSealed && skill.Magical)
                return;

            //ECOKEY CA的魔法封印
            if (actor.Buff.Sealed && skill.Physical)
                return;
            ...
            if (target != 0xFFFFFFFF)
            {
                ...
                if (GetLengthD(this.Mob.X, this.Mob.Y, dactor.X, dactor.Y) <= Math.Abs(skill.Range) * 145 || skill.Range == -1)//ECOKEY 判斷修正（只改這句）
                {
                    if (skill.Target == 2)
                    {
                        //如果是辅助技能
                    ...
                    if (this.master != null)//ECOKEY 召喚物判斷更改
                    {
            ...
            if (skill.CastTime > 0)
            {
                ...
                if (skill.Physical)
                    actor.TInt["skilltype"] = 1;
                else if (skill.Magical)
                    actor.TInt["skilltype"] = 2;
        ...
        public void AttackActor(uint actorID)
        {
            if (this.Hate.ContainsKey(actorID))
                ...
                this.Hate.TryAdd(actorID, this.Mob.MaxHP);//二哈更改Hate
        }
            ...
            try
            {
                ...
                for (uint i = 0; i < this.Hate.Keys.Count; i++)//Find out the actorPC with the highest hate value
                {
                    ...
                    if (hate < this.Hate[ids[i]])
                    {
                        ...
                        if (tmp == null)
                        {
                            this.Hate.TryRemove(id, out _);//二哈更改Hate
                            id = 0;
                            hate = 0;
                            i = 0;
                            continue;
                ...
                if (id != 0)//Now the id is refer to the PC with the highest hate to the Mob.现在这个ID是怪物对最高仇恨者的ID
                {
                    ...
                    if (tmp != null)
                    {
                        if (tmp.HP == 0)
                        {
                            this.Hate.TryRemove(tmp.ActorID, out _);//二哈更改Hate
                            id = 0;
                        }
                    }
                }
        ...
        public void OnAttacked(Actor sActor, int damage) //2/6/2024
        {
            ...
            if (this.Hate != null)
            {
                ...
                uint HateTargetId = sActor.ActorID;

                if (sActor.Status.Additions.ContainsKey("误导") && sActor.TInt["误导"] != 0 && map.GetActor((uint)sActor.TInt["误导"]) != null)
                {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        this.Hate.TryAdd(HateTargetId, tmp);//二哈更改Hate
                    }
                }
                catch (Exception ex)
                {
                    // 處理例外情況，例如記錄錯誤信息或採取其他適當的措施
                    SagaLib.Logger.ShowError("處理 Hate 字典時出現錯誤: " + ex.ToString());
                }
            }
            ...
            if (damage > 0)
            {
                ...
                if (sActor.type == ActorType.PC)
                {
                    ...
                    else this.DamageTable.TryAdd(sActor.ActorID, damage);

                    //寵物傷害判斷
                    ActorPC master = (ActorPC)sActor;
                    if (master.Partner != null)
                    {
                        ActorPartner par = (ActorPartner)master.Partner;
                        //new
                        if (this.DamageTable.ContainsKey(par.ActorID))
                        // old
                        // if (this.DamageTable.ContainsKey(par.ActorID) && this.DamageTable[par.ActorID] != 1)
                        {
                            if (this.DamageTable[par.ActorID] != 1)
                            {
                                this.DamageTable[master.ActorID] += this.DamageTable[par.ActorID];
                                //this.DamageTable.Remove(par.ActorID);
                                this.DamageTable[par.ActorID] = 1;
                            }
                        }
                        else this.DamageTable.TryAdd(par.ActorID, 1);
                ...
                {
                    ActorPartner par = (ActorPartner)sActor;
                    ActorPC master = (ActorPC)par.Owner;


                    lock (this.DamageTable)
                    {
                        if (par.Owner.Pet != null && par.Owner.Pet.Ride == true)
                        {

                            if (this.DamageTable.ContainsKey(master.ActorID))
                            {
                                this.DamageTable[master.ActorID] += damage;
                            }
                            else
                            {
                                this.DamageTable.TryAdd(master.ActorID, damage);
                            }
                        }
                        else
                        {

                            if (master.TimerItems.ContainsKey("PARTNER_GET_EXP")) //ECOKEY 常駐Task，限時道具
                            {
                                if (this.DamageTable.ContainsKey(master.ActorID))
                                {
                                    this.DamageTable[master.ActorID] += damage;
                            ...
                            else
                            {
                                if (this.DamageTable.ContainsKey(par.ActorID))
                                {
                                    this.DamageTable[par.ActorID] += damage;
                                }
                                else
                                {
                                    this.DamageTable.TryAdd(par.ActorID, damage);
                                }
                        ...
                        if (this.DamageTable.TryGetValue(master.ActorID, out var masterDamage))
                        {
                            if (masterDamage > Mob.MaxHP)
                            {
                                this.DamageTable[master.ActorID] = (int)Mob.MaxHP;
                            }
                        }

                        if (this.DamageTable.TryGetValue(par.ActorID, out var partnerDamage))
                        {
                            if (partnerDamage > Mob.MaxHP)
                            {
                                this.DamageTable[par.ActorID] = (int)Mob.MaxHP;
                            }
                        }
                ...
                /*  else if (sActor.type == ActorType.PARTNER)
                  {
                      ...
                      if (par.Owner.Pet != null && par.Owner.Pet.Ride == true)
                      {
                          ...
                          else this.DamageTable.Add(master.ActorID, damage);
                          if (this.DamageTable.ContainsKey(master.ActorID))
                          {
                              if (this.DamageTable[master.ActorID] > Mob.MaxHP)
                                  this.DamageTable[master.ActorID] = (int)Mob.MaxHP;
                          }
                      }
                      //ECOKEY 寵物處理
                      else
                      {
                          if (master.TInt["DogHead"] == 1)//狗頭開著
                          {
                              ...
                              else this.DamageTable.Add(par.ActorID, 1);
                          }
                          else
                          {
                              if (this.DamageTable.ContainsKey(master.ActorID))
                              ...
                              else
                              {
                                  ...
                                  else this.DamageTable.Add(par.ActorID, damage);
                              }
                          }
                          ...
                          if (this.DamageTable.ContainsKey(par.ActorID))
                          {
                              if (this.DamageTable[par.ActorID] > Mob.MaxHP)
                                  this.DamageTable[par.ActorID] = (int)Mob.MaxHP;
                          }
                      }
                  }*/
                ...
                /*  else if (sActor.type == ActorType.MOB)
                  {
                      ActorMob parMob = (ActorMob)sActor;
                      if (parMob.Owner != null && parMob.Owner.type == ActorType.PC)
                      {
                          ActorPC master = (ActorPC)parMob.Owner;


                          lock (this.DamageTable)
                          {

                              if (master.TInt["DogHead"] == 1)
                              {
                                  if (this.DamageTable.ContainsKey(master.ActorID))
                                  {
                                      this.DamageTable[master.ActorID] += damage;
                                  ...
                                  {
                                      this.DamageTable.Add(master.ActorID, damage);
                                  }
                              }
                              else
                              {

                                  if (this.DamageTable.ContainsKey(parMob.ActorID))
                                  {
                                      this.DamageTable[parMob.ActorID] += damage;
                                  }
                                  else
                                  {
                                      this.DamageTable.Add(parMob.ActorID, damage);
                                  }
                              }


                              if (this.DamageTable.ContainsKey(master.ActorID) && this.DamageTable[master.ActorID] > Mob.MaxHP)
                                  this.DamageTable[master.ActorID] = (int)Mob.MaxHP;

                              if (this.DamageTable.ContainsKey(parMob.ActorID) && this.DamageTable[parMob.ActorID] > Mob.MaxHP)
                                  this.DamageTable[parMob.ActorID] = (int)Mob.MaxHP;
                ...
                else if (sActor.type == ActorType.MOB)
                {
                    ...
                    if (parMob.Owner != null && parMob.Owner.type == ActorType.PC)
                    {
                        ...
                        {
                            if (parMob.MobID == 26040001 || parMob.MobID == 26100009 || parMob.MobID == 26010000 || parMob.MobID == 26070003)//木偶王除外
                            {
                                if (this.DamageTable.ContainsKey(master.ActorID))
                                {
                                    this.DamageTable[master.ActorID] += damage;
                                ...
                                {
                                    this.DamageTable.TryAdd(master.ActorID, damage);
                                    if (firstAttacker == null) firstAttacker = master;
                                }
                            }
                            else if (master.TimerItems.ContainsKey("PARTNER_GET_EXP"))//ECOKEY 常駐Task，限時道具
                            {
                                if (this.DamageTable.ContainsKey(master.ActorID))
                                {
                                    this.DamageTable[master.ActorID] += damage;
                                }
                                else this.DamageTable.TryAdd(master.ActorID, damage);
                            }
                            else
                            {
                                if (this.DamageTable.ContainsKey(master.ActorID))
                                {
                                    ...
                                    if (this.DamageTable.ContainsKey(parMob.ActorID))
                                    {
                                        this.DamageTable[master.ActorID] += this.DamageTable[parMob.ActorID];
                                        this.DamageTable.TryRemove(parMob.ActorID, out _);
                                    }
                                }
                                else
                                {
                                    ...
                                    else this.DamageTable.TryAdd(parMob.ActorID, damage);
                                }
                            }
                        }
                        ...
                        if (this.DamageTable.ContainsKey(parMob.ActorID))
                        {
                            if (this.DamageTable[parMob.ActorID] > Mob.MaxHP)
                                this.DamageTable[parMob.ActorID] = (int)Mob.MaxHP;
                        }
                    }
                }
        ...
        /*   public void OnAttacked(Actor sActor, int damage) old
           {
               if (this.actor.Buff.Dead)
                   return;
               if (this.Activated == false)
       ...
       */
        ...
        public void OnSeenSkillUse(SkillArg arg)
        {
            ...
            /*   if (actor.Buff.Transparent == true || arg.skill.ID == 3097)
               {
                   // 隱身技能處理邏輯
                   this.Hate.Clear(); // 清除仇恨目標
                   return;
               }*/
            /* if (actor != null && (actor.Buff.Transparent == true || arg != null && arg.skill.ID == 3097))
             {
                 // 隱身技能處理邏輯
                 this.Hate.Clear(); // 清除仇恨目標
                 return;
            ...
            if (master != null)
            {
                for (int i = 0; i < arg.affectedActors.Count; i++)
                {
                    if (arg.affectedActors[i].ActorID == master.ActorID)
                    {
                        ...
                        if (actor != null && actor.type != ActorType.PARTNER)//ECOKEY 修復召喚物攻擊寵物問題
                        {
