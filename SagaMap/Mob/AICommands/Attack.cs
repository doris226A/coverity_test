    ...
    public class Attack : AICommand
    {
        ...
        private Actor CurrentTarget()
        {
            try
            {
                ...
                {
                    //new 12.15
                    List<uint> ids = new List<uint>(mob.Hate.Keys);
                    if (ids.Count > 0)
                    {
                        uint[] idsArray = ids.ToArray();
                        ...
                        //for (uint i = 0; i < mob.Hate.Keys.Count; i++)//Find out the actorPC with the highest hate value
                        {
                            ...
                            if (hate < mob.Hate[ids[i]])
                            {
                                ...
                                if (tmp == null)
                                {
                                    mob.Hate.TryRemove(id, out _);//二哈更改Hate
                                    id = 0;
                                    hate = 0;
                                    active = false;
                                    i = 0;
                                ...
                                //控制partner不被攻击的语句，因为BUG的缘故，暂时继续让partner被封锁
                                ...
                                if (tmp.type == ActorType.PARTNER)
                                {
                                    ActorPartner p = (ActorPartner)tmp;
                                    if (this.mob.Master != null && p.Owner.ActorID == this.mob.Master.ActorID)
                                    {
                                        mob.Hate.TryRemove(id, out _);//二哈更改Hate
                                        continue;
                                    }
                                }
                                //ECOKEY 召喚物不打玩家（騎士團除外）
                                if (tmp.type == ActorType.PC)
                                {
                                    ActorPC p = (ActorPC)tmp;
                                    if (this.mob.Master != null)
                                    {
                                        if (p.ActorID == this.mob.Master.ActorID)
                                        {
                                            continue;
                                        }
                                        if (!Skill.SkillHandler.Instance.CheckValidAttackTarget(mob.Mob, p))
                                        {
                                            mob.Hate.TryRemove(id, out _);//二哈更改Hate
                                            continue;
                                        }
                    ...
                    if (id != 0)//Now the id is refer to the PC with the highest hate to the Mob.现在这个ID是怪物对最高仇恨者的ID
                    {
                        ...
                        if (tmp != null)
                        {
                            if (tmp.HP == 0)
                            {
                                mob.Hate.TryRemove(tmp.ActorID, out _);//二哈更改Hate
                                id = 0;
                                active = false;
                            }
                        }
                        ...
                        if (Amob.PriorityTartget != null)
                        {
                            if (Amob.MapID == Amob.PriorityTartget.MapID && Amob.HP > 0 && Amob.PriorityTartget.HP > 0)
                            {
                                ...
                                if (!mob.Hate.ContainsKey(Amob.PriorityTartget.ActorID))
                                    mob.Hate.TryAdd(Amob.PriorityTartget.ActorID, 5);//二哈更改Hate
        ...
        private void CheckAggro()
        {
            ...
            if (this.mob.Master != null)
            {
                if (!this.mob.Hate.ContainsKey(this.mob.Master.ActorID))
                    this.mob.Hate.TryAdd(this.mob.Master.ActorID, 1);//二哈更改Hate
            ...
            foreach (uint id in ids)
            {
                /*  if (mob.map == null)
                  ...
                  if (i.Status.Additions.ContainsKey("Through"))
                      continue;
                  //ECOKEY 未知結界
                  if (i.Status.Additions.ContainsKey("HexaGram"))
                      continue;*/


                // 檢查是否可以獲取到 Actor
                Actor i = mob.map.GetActor(id);
                if (i == null || i.Status == null || i.Status.Additions == null)
                    continue;
                ...
                // 排除 Buffs
                if (i.Buff.Transparent || i.Status.Additions.ContainsKey("IAmTree") || i.Status.Additions.ContainsKey("Through"))
                    continue;
                ...
                if (mob.Mob.type != ActorType.PC && i.type == ActorType.MOB)
                {
                    ...
                    if (isSlavaOfPc && eh.AI.Mode.Symbol)//ECOKEY 禁止打象徵
                        continue;
                    if (eh.AI.Mode.Symbol && !isSlavaOfPc)
                    {
                        mob.Hate.TryAdd(i.ActorID, 20);//二哈更改Hate
                        SendAggroEffect();
                    }
                }
                ...
                if (isSlavaOfPc && i.type == ActorType.MOB)
                {
                    ...
                    if (ie.AI.Master == this.mob.Master)
                        continue;
                    //ECOKEY 新增騎士團象徵判斷
                    if (((ActorPC)this.mob.Master).Mode == PlayerMode.KNIGHT_SOUTH && ((ActorMob)i).MobID == 40053000) continue;
                    if (((ActorPC)this.mob.Master).Mode == PlayerMode.KNIGHT_NORTH && ((ActorMob)i).MobID == 40054000) continue;
            ...
            if (distance <= r)
            {
                ...
                if (!mob.Hate.ContainsKey(target.ActorID))
                    mob.Hate.TryAdd(target.ActorID, 20);//二哈更改Hate
                SendAggroEffect();
            }
        }
        ...
        public void Update(object para)
        {
            try
            {
                // 在方法开头检查 attacktask 是否为 null，如果是，则进行初始化
                if (attacktask == null)
                {
                    attacktask = new MobAttack(mob, dest);
                }
                ...
                if (pet != null)
                    if (!this.mob.Hate.ContainsKey(pet.Owner.ActorID))
                        this.mob.Hate.TryAdd(pet.Owner.ActorID, 1);//二哈更改Hate
                if (this.mob.Master != null)
                {
                    if (!this.mob.Hate.ContainsKey(this.mob.Master.ActorID))
                        this.mob.Hate.TryAdd(this.mob.Master.ActorID, 1);//二哈更改Hate
                ...
                {
                    if (this.mob.Master != null && this.dest == this.mob.Master)//ECOKEY 召喚物技能判斷（避免對主人攻擊）
                    {
                ...
                if (dest.HP == 0)
                {
                    if (pet != null)
                    {
                        ...
                        {
                            if (mob.Hate.ContainsKey(dest.ActorID)) mob.Hate.TryRemove(dest.ActorID, out _);//二哈更改Hate
                            if (attacktask.Activated == true) attacktask.Deactivate();
                            attacktask = null;
                            return;
                        }
