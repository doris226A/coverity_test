    ...
    public partial class StatusFactory : Singleton<StatusFactory>
    {
        ...
        private void CalcPartnerEquipBonus(ActorPartner partner)
        {
            ...
            foreach (EnumPartnerEquipSlot j in equips.Keys)
            {
                ...
                partner.Status.def_item = (short)(partner.Status.def_item + i.BaseData.def + i.Def); //装备提供右防
                partner.Status.mdef_item = (short)(partner.Status.mdef_item + i.BaseData.mdef + i.MDef);  //装备提供右防

                partner.Status.def_add_item = (short)(partner.Status.def_add_item + i.BaseData.luk + i.Def); //ECOKEY 真˙左防（%）
                partner.Status.mdef_add_item = (short)(partner.Status.mdef_add_item + i.BaseData.cha + i.MDef);  //ECOKEY 真˙左防（%）
