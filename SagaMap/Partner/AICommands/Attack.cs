...
namespace SagaMap.Partner.AICommands
{
    public class Attack : AICommand
    {
        ...
        {
            //ECOKEY 寵物AI重寫0623，新增初始數據
            partnerai.ai_lastSkillCast.Clear();
            partnerai.ai_lastSkillCast.Add(0, DateTime.Now);
            partnerai.ai_lastSkillCast.Add(1, DateTime.Now);
            partnerai.ai_lastSkillCast.Add(2, DateTime.Now);
            partnerai.ai_lastSkillCast.Add(3, DateTime.Now);
            partnerai.ai_lastSkillCast.Add(4, DateTime.Now);
            partnerai.ai_lastSkillCast.Add(5, DateTime.Now);
            partnerai.ai_lastSkillCast.Add(6, DateTime.Now);
            partnerai.ai_lastSkillCast.Add(7, DateTime.Now);
            partnerai.ai_lastSkillCast.Add(8, DateTime.Now);
            partnerai.ai_lastSkillCast.Add(9, DateTime.Now);
            //ECOKEY直接新增初始數據
            ...
            try
            {
                ...
                for (uint i = 0; i < partnerai.Hate.Keys.Count; i++)//Find out the actorPC with the highest hate value
                {
                    ...
                    if (hate < partnerai.Hate[ids[i]])
                    {
                        ...
                        if (tmp == null)
                        {
                            partnerai.Hate.TryRemove(id, out _);//二哈更改Hate
                            id = 0;
                            hate = 0;
                            active = false;
                            i = 0;
                ...
                if (id != 0)//Now the id is refer to the PC with the highest hate to the Mob.现在这个ID是怪物对最高仇恨者的ID
                {
                    ...
                    if (tmp != null)
                    {
                        ...
                        {
                            partnerai.Hate.TryRemove(tmp.ActorID, out _);//二哈更改Hate
        ...
        private void CheckAggro()
        {
            ...
            if (this.partnerai.Master != null)
            {
                if (!this.partnerai.Hate.ContainsKey(this.partnerai.Master.ActorID))
                    this.partnerai.Hate.TryAdd(this.partnerai.Master.ActorID, 1);
            ...
            for (int index = 0; index < actorIds.Length; index++) // Use a for loop with index
            {
                ...
                if (i.Status.Additions.ContainsKey("Through"))
                    continue;
                if (i.HP == 0)//&& i.type != ActorType.ITEM)//自動撿
                    continue;
                if (i.type == ActorType.MOB)
                {
                    ...
                    if (eh.AI.Mode.Symbol)//ECOKEY 禁止打象徵
                        continue;
                }
            ...
            if (distance <= 1000)
            {
                ...
                if (!partnerai.Hate.ContainsKey(target.ActorID))
                    partnerai.Hate.TryAdd(target.ActorID, 20);
                SendAggroEffect();
            }
        }
        ...
        public void Update(object para)
        {
            ...
            if (partner != null)
            {
                ...
                if (partner.Owner != null)
                {
                    ...
                    {
                        partnerai.Hate.TryAdd(partner.Owner.ActorID, 1); // 如果該鍵不存在，則添加新的鍵值對
                    }
                    else
                    {
            ...
            /*  if (partner != null)
              {
                  if (!partnerai.Hate.ContainsKey(partner.Owner.ActorID))
                  {
                      partnerai.Hate.TryAdd(partner.Owner.ActorID, 1);
                  }
              }*/

            if (partnerai.Master != null)
                if (!partnerai.Hate.ContainsKey(this.partnerai.Master.ActorID))
                    partnerai.Hate.TryAdd(this.partnerai.Master.ActorID, 1);
            ...
            if (partner.ai_mode == 3)
            {
                if ((DateTime.Now - partnerai.LastSkillCast).TotalSeconds >= 1)
                {
                    partnerai.OnShouldCastSkill_AI(dest);
                    partnerai.LastSkillCast = DateTime.Now;
                }
            }
            else if (partner.ai_mode == 4)
            {
                if ((DateTime.Now - partnerai.LastSkillCast).TotalSeconds >= 1)
                {
                    partnerai.OnShouldCastSkill_AI2(dest);
                    partnerai.LastSkillCast = DateTime.Now;
                }
            }
            ...
            if (dest.HP == 0)
            {
                if (partner != null)
                {
                    ...
                    {
                        if (partnerai.Hate.ContainsKey(dest.ActorID)) partnerai.Hate.TryRemove(dest.ActorID, out _);//二哈更改Hate
        ...
        void CustomSkill2(Actor currentTarget)
        {
            ...
            {
                if (partner.ai_reactions2.ContainsKey(i))//0705再更新，注意此處更新是AI2
                {
        ...
        {
            ActorPartner partner = (ActorPartner)this.partnerai.Partner;
            if (((ActorEventHandlers.PartnerEventHandler)partner.e).AI.Mode.mask.Test(AIFlag.NoMove)) return;
