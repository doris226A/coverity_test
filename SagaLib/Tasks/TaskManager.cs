...
namespace SagaLib
{
    public class TaskManager : Singleton<TaskManager>
    {
        ...
        {
            // 確保在清除任務時，能夠正確處理所有的線程和任務。
            lock (registered)
            {
                foreach (var task in registered.ToArray())
                {
                    try
                    {
                        task.Deactivate(); // 禁用任務
                    }
                    catch (Exception ex)
                    {
                        Logger.ShowError($"Error deactivating task: {ex}");
                    }
                }
                registered.Clear(); // 從集合中清除所有任務
            }

            // 清空隊列中的所有任務
            while (fifo.TryDequeue(out var _)) { }
            while (slowFifo.TryDequeue(out var _)) { }

            // 重置任務計數器
            exeCount = 0;
            exeTime = 0;
            ExecutionCountPerMinute = 0;
        ...
        public void ClearTask()
        {
            ...
            lock (registered)
            {
                foreach (MultiRunTask i in registered.ToArray())
                {
                    i.Deactivate();
                }
            }
