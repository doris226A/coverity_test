...
//#endif
     ...
     /*   public static void RemoveThread(string name)
        {
            lock (Threads)
            {
                if (Threads.ContainsKey(name)) // 检查要移除的线程是否存在
                {
                    Threads.Remove(name); // 执行移除操作
                }
                else
                {
                    // 警告或记录一个消息，表示要移除的线程不存在
                    Console.WriteLine($"Thread '{name}' does not exist. No action taken.");
                }
            }
        }*/
          ...
          public static void RemoveThread(string name)
          {
              if (Threads.ContainsKey(name)) // 檢查要移除的線程是否存在
              {
                  lock (Threads)
                  {
                      Threads.Remove(name); // 執行移除操作
                  }
              }
              else  
              {
                  // 警告或記錄一個訊息，表示要移除的線程不存在
                  Console.WriteLine($"Thread '{name}' does not exist. No action taken.");
              }
              /*  if (Threads.ContainsKey(name))
                {
        ...
        {
            // 设置定时器，在一定时间后检测死锁
            deadlockTimer = new Timer(CheckAndResolveDeadlock, null, TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(10));

            while (true)
            {
                Thread.Sleep(5000); // 等待一段时间再次检测
            }
        }

        private void CheckAndResolveDeadlock(object state)
        {
            if (enteredcriarea && !Debugger.IsAttached)
            {
                TimeSpan span = DateTime.Now - timestamp;
                if (span.TotalSeconds > 10 && !noCheckDeadLock)
                {
                    Console.WriteLine("Deadlock detected");
                    Console.WriteLine("Automatically unlocking....");

                    UnlockBlockedThreads();
                }
            }
        }

        private void UnlockBlockedThreads()
        {
            lock (blockedThread) // 添加锁，以确保在操作列表时是线程安全的
            {
                foreach (Thread blocked in blockedThread)
                {
                    try
                    {
                        if (blocked.ThreadState != System.Threading.ThreadState.Running)
                            blocked.Resume();
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"Error occurred while unlocking thread: {ex.Message}");
                    }
                }
  ...
  //#endif
        ...
        //ECOKEY 可覆蓋，將ShowWarninge改成ShowInfo
        public static void PrintAllThreads()
        {
            Logger.ShowInfo("Call Stack of all blocking Threads:");
            Thread[] list = blockedThread.ToArray();
            foreach (Thread j in list)
            {
                try
                {
                    Logger.ShowInfo("Thread name:" + getThreadName(j));
                    j.Suspend();
                    StackTrace running = new StackTrace(j, true);
                    j.Resume();
                    foreach (StackFrame i in running.GetFrames())
                    {
                        Logger.ShowInfo("at " + i.GetMethod().ReflectedType.FullName + "." + i.GetMethod().Name + " " + i.GetFileName() + ":" + i.GetFileLineNumber());
                    }
                }
                catch { }
                Console.WriteLine();
            }
            Logger.ShowInfo("Call Stack of all Threads:");
            string[] keys = new string[Threads.Keys.Count];
            Threads.Keys.CopyTo(keys, 0);
            foreach (string k in keys)
            {
   ...
   public static void LeaveCriticalArea(Thread blocker)
          {
              if (blockedThread.Contains(blocker) || blockedThread.Count != 0)
              {
                  ...
                  if (blockedThread.Contains(blocker))
                  {
                      ...
                      {
                         // Logger.ShowError($"Error occurred while removing thread from blockedThread list: {ex}");
                  ...
                  {
                    //  Logger.ShowError("Cannot remove thread: blockedThread list does not contain the specified thread!");
                  }
