...
namespace SagaDB.Synthese
{
    public class SyntheseFactory : Factory<SyntheseFactory, SyntheseInfo>
    {
        ...
        protected override void ParseCSV(SyntheseInfo item, string[] paras)
        {
            ...
            item.GetExpMinLevel = byte.Parse(paras[1]);
            item.GetExpMaxLevel = byte.Parse(paras[2]);
            item.GetExpMinJobLevel = byte.Parse(paras[3]);
            item.GetExpMaxJobLevel = byte.Parse(paras[4]);
            ...
            {
                ItemElement tmp = new ItemElement();
                var tempdata = paras[i].Split(':');
                tmp.ID = uint.Parse(paras[i].Split(':')[0]);
                if (paras[i].Split(':').Length == 2)
                    tmp.ItemSkillID = ushort.Parse(paras[i].Split(':')[1]);
                tmp.Count = ushort.Parse(paras[i + 1]);
