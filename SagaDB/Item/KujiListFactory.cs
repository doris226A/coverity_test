...
namespace SagaDB.Item
{
    public class KujiListFactory : Singleton<KujiListFactory>
    {
        ...
        public Dictionary<uint, uint> ItemChoiceList = new Dictionary<uint, uint>(); //ECOKEY
        ...
        public void InitChoiceList(string path, Encoding encoding) //ECOKEY
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                try
                {
                    ...
                    if (paras[1] == "") continue; // 商品ID所在的列
                    uint Itemid = uint.Parse(paras[1]); // 商品ID所在的列 
                    uint Price = uint.Parse(paras[3]); // 价格所在的列
