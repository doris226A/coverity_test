...
namespace SagaDB.Item
{
    ...
    public class Inventory
    {
        ...
        public Inventory(Actor.ActorPC owner)
        {
            ...
            ware.Add(WarehousePlace.MainFGarden, new List<Item>());
            ware.Add(WarehousePlace.SubFGarden, new List<Item>());
            ware.Add(WarehousePlace.Chips, new List<Item>());
            ware.Add(WarehousePlace.IrisCardHolder, new List<Item>());
        ...
        public void CalcPayloadVolume()
        {
            ...
            uint pal = 0, vol = 0;
            if (this.Owner.Skills3.ContainsKey(1112))//ECOKEY ST寶物的奧秘
            {
                float bouns = new float[] { 0, 2.5f, 3.6f, 4.0f, 5.0f, 10.0f }[this.Owner.Skills3[1112].Level];
                foreach (Item i in list)
                {
                    if (i.BaseData.itemType == ItemType.TIMBER_BOX || i.BaseData.itemType == ItemType.TREASURE_BOX || i.BaseData.itemType == ItemType.CONTAINER)
                    {
                        pal += (uint)(i.BaseData.weight / bouns * i.Stack);
                        vol += (uint)(i.BaseData.volume / bouns * i.Stack);
                    }
                    else
                    {
        ...
        public InventoryAddResult AddWareItem(WarehousePlace place, Item item)
        {
            try
            {
                //ECOKEY 道具堆疊999優化（整段）
                ushort count = 9999;
                if (this.owner.Online) count = 999;
                needSaveWare = true;
                if (item.Stack > 0)
                {
                ...
                if (query.Count() != 0 && item.Stackable)
                {
                    ...
                    {
                        ushort rest = (ushort)(oriItem.Stack - count);
                        if (rest > count)
                        {
                            Logger.ShowWarning("Adding too many item(" + item.BaseData.name + ":" + item.Stack + "), setting count to the maximal value(count)");
                            rest = count;
                        }
                        oriItem.Stack = count;
                        item.Stack = oriItem.Stack;
        ...
        public Item GetItem2(uint itemID)
        {
            List<Item> list = items[ContainerType.BODY];
            List<Item> result = new List<Item>();
            result = list.Where(x => x.ChangeMode && x.ItemID == itemID).ToList();
            if (result.Count() == 0)
            {
                return null;
            }
        ...
        public byte[] WareToBytes(List<Item> lst)
        {
            System.IO.MemoryStream ms = new System.IO.MemoryStream();
            System.IO.BinaryWriter bw = new System.IO.BinaryWriter(ms);
            foreach (Item j in lst)
            {
                j.ToStream(ms);
            }
            ms.Flush();
