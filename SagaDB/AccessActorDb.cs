...
namespace SagaDB
{
    public class AccessActorDb : AccessConnectivity, ActorDB
    {
        ...
        void ActorDB.SaveItem(ActorPC pc)
        {
        ...
        public List<ActorPC> GetFriendList(ActorPC pc)
        {
            ...
            for (int i = 0; i < result.Count; i++)
            {
                ...
                uint currentMap = (uint)result[i]["currentMap"]; // Read the currentMap value
                chara.MapID = currentMap;

                sqlstr = "SELECT `name`,`job`,`lv`,`jlv1`,`jlv2x`,`jlv2t`,`mapID` FROM `char` WHERE `char_id`='" + friend + "' LIMIT 1;";
        ...
        public void SaveFGarden(ActorPC pc)
        {
            throw new NotImplementedException();
        }
