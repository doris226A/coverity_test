...
namespace SagaDB.Partner
{
    public class PartnerFactory : Singleton<PartnerFactory>
    {
        ...
        public PartnerCubeType GetCubeType(ushort uniqueID)
        {
            if (actcubes_db_uniqueID.ContainsKey(uniqueID))
            {
                return actcubes_db_uniqueID[uniqueID].cubetype;
            }
            return PartnerCubeType.UNKNOWN; // 或者根據需要返回適當的值
        }
        ...
        public void InitPartnerInfo(string path,System.Text.Encoding encoding)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                try
                {
                    ...
                    partner.ridingflag = toBool(paras[5]);
                    try
                    {
                    ...
                    {
                        Logger.ShowError("重複的PartnerID:" + partner.id.ToString() + ",[" + partners_info[partner.id].name + "]的已存在" + ",讓[" + partner.name + "]沒有被添加到Info。 ");
                    }
                    else
                    {
        ...
        public void InitPartnerRankDB(string path, System.Text.Encoding encoding)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                try
                {
                    ...
                    byte baserank = 0; // byte.Parse(paras[1]);
        ...
        public void InitPartnerDB(string path, System.Text.Encoding encoding)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                try
                {
                    ...
                    partner.isrange = toBool(paras[11]);
                    partner.range = float.Parse(paras[12]);
                    partner.canattack = toBool(paras[13]);
                    partner.damagerate = ushort.Parse(paras[14]);
                    partner.commonai = ushort.Parse(paras[15]);
                    partner.dyingai = ushort.Parse(paras[16]);

                    for (int i = 17; i < 25; i++)
                    {
                        partner.born_skills.Add(uint.Parse(paras[i]));
                    }

                    for (int i = 25; i < 35; i++)
                    {
                        partner.born_active_cubes.Add(uint.Parse(paras[i]));
                    }

                    for (int i = 35; i < 40; i++)
                    {
                        partner.born_passive_cubes.Add(uint.Parse(paras[i]));
                    }

                    partner.possessionflag = toBool(paras[40]);
                    partner.possessionskillid = ushort.Parse(paras[41]);
                    partner.possessionstatusuprate = ushort.Parse(paras[42]);
        ...
        public void InitPartnerFoodDB(string path, System.Text.Encoding encoding)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                try
                {
                    ...
                    {
                        Logger.ShowError("重複的PartnerFoodID:" + food.itemID.ToString() + ",已存在,沒有被添加到DB。 ");
                    }
                    else
                    {
        ...
        public void InitPartnerEquipDB(string path, System.Text.Encoding encoding)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                try
                {
                    ...
                    item.BaseData.luk = short.Parse(paras[8]);//新增左防-借用luk
                    item.BaseData.mdef = short.Parse(paras[9]);
                    item.BaseData.cha = short.Parse(paras[10]);//新增左防-借用cha
                    item.BaseData.hitMelee = short.Parse(paras[11]);
                    item.BaseData.hitRanged = short.Parse(paras[12]);
                    item.BaseData.hitMagic = short.Parse(paras[13]);
                    item.BaseData.avoidMelee = short.Parse(paras[14]);
                    item.BaseData.avoidRanged = short.Parse(paras[15]);
                    item.BaseData.avoidMagic = short.Parse(paras[16]);
                    item.BaseData.hitCritical = short.Parse(paras[17]);
                    item.BaseData.avoidCritical = short.Parse(paras[18]);
                    ...
                    {
                        Logger.ShowError("重複的PartnerEquipID:" + equip.itemID.ToString() + ",已存在,沒有被添加到DB。 ");
                    }
                    else
                    {
        ...
        public void InitActCubeDB(string path, System.Text.Encoding encoding)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                try
                {
                    ...
                    //ECOKEY 新增Rank讀取
                    cube.partnerrank = byte.Parse(paras[4]); //[5]-[15] is system related 0s
                    cube.systemID = byte.Parse(paras[5]); //[5]-[15] is system related 0s
