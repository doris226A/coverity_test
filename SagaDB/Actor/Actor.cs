...
namespace SagaDB.Actor
{
    ...
    public partial class Actor
    {
        ...
        [NonSerialized]
        Dictionary<string, SagaLib.MultiRunTask> tasks_online = new Dictionary<string, SagaLib.MultiRunTask>();//ECOKEY 常駐Task
        [NonSerialized]
        Dictionary<string, SagaLib.MultiRunTask> tasks = new Dictionary<string, SagaLib.MultiRunTask>();
        [NonSerialized]
        ConcurrentDictionary<SagaLib.Elements, int> elements = new ConcurrentDictionary<SagaLib.Elements, int>();//二哈更改測試-屬性
        [NonSerialized]
        ...
        /// 常駐Task，直到下線都不會被清除
        /// </summary>
        public Dictionary<string, SagaLib.MultiRunTask> Tasks_Online { get { return this.tasks_online; } }//ECOKEY 常駐Task
        ...
        /// 防御元素属性值
        /// </summary>
        public ConcurrentDictionary<SagaLib.Elements, int> Elements//二哈更改測試-屬性
        {
            get
            {
                if (this.elements.Count == 0)
                {
                    this.elements.TryAdd(SagaLib.Elements.Neutral, 0);
                    this.elements.TryAdd(SagaLib.Elements.Fire, 0);
                    this.elements.TryAdd(SagaLib.Elements.Water, 0);
                    this.elements.TryAdd(SagaLib.Elements.Wind, 0);
                    this.elements.TryAdd(SagaLib.Elements.Earth, 0);
                    this.elements.TryAdd(SagaLib.Elements.Holy, 0);
                    this.elements.TryAdd(SagaLib.Elements.Dark, 0);
                }
        ...
        {
            // 创建 this.Tasks 的副本
            List<MultiRunTask> tasksCopy = new List<MultiRunTask>(this.Tasks.Values);

            // 遍历副本
            foreach (MultiRunTask i in tasksCopy)
            {
                try
                {
                    i.Deactivate();
        ...
        public void ClearAddition()
        {
            ...
            foreach (Addition i in additionlist)
            {
                try
                {
                    if (i.EcoKeySkillType != Addition.AdditionType.PassiveSkill)
                    {
                        if (i.Activated)
                        {
                            i.AdditionEnd();
                            this.Status.Additions.TryRemove(i.Name, out _);
                        }
                    }
                }
                catch
        ...
        /// 清除該Actor所有常駐狀態（下線專用）
        /// </summary>
        public void ClearTaskOnline()
        {
            List<MultiRunTask> tasksCopy = new List<MultiRunTask>(this.Tasks_Online.Values);
            foreach (MultiRunTask i in tasksCopy)
            {
                try
                {
