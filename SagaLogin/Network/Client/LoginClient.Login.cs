...
namespace SagaLogin.Network.Client
{
    public partial class LoginClient : SagaLib.Client
    {
        ...
        public void OnLogin(Packets.Client.CSMG_LOGIN p)
        {
            ...
            {
                SendLoginAck(SagaLogin.Packets.Server.SSMG_LOGIN_ACK.Result.GAME_SMSG_LOGIN_ERR_IPBLOCK);
                return;
            }
            ...
            if (LoginServer.accountDB.CheckPassword(p.UserName, p.Password, this.frontWord, this.backWord))
            {
                ...
                // 檢查是否已經有相同帳號的玩家在線上
                var existingClient = LoginClientManager.Instance.FindClientAccount(p.UserName);
                if (existingClient != null && tmp.GMLevel == 0)
                {
                    existingClient.netIO.Disconnect();
                    SendLoginAck(SagaLogin.Packets.Server.SSMG_LOGIN_ACK.Result.GAME_SMSG_LOGIN_ERR_ALREADY);
                    return;
                }
                ...
                if (account.Banned)
                {
                    SendLoginAck(SagaLogin.Packets.Server.SSMG_LOGIN_ACK.Result.GAME_SMSG_LOGIN_ERR_BFALOCK);
                    return;
                }

                // 立即確認登入成功，後續操作異步處理
                SendLoginAck(SagaLogin.Packets.Server.SSMG_LOGIN_ACK.Result.OK);

                // 檢查帳號名稱是否包含非法字元
                if (p.UserName.Contains(" ") || p.UserName.Contains("\\"))
                {
                    SendLoginAck(SagaLogin.Packets.Server.SSMG_LOGIN_ACK.Result.GAME_SMSG_LOGIN_ERR_UNKNOWN_ACC);
                    return;
                }
                ...
                // 檢查是否有其他角色在線上
                if (LoginClientManager.Instance.FindClient(account.PlayerNames) != null)
                {
                    LoginClientManager.Instance.FindClient(account.PlayerNames).netIO.Disconnect();
                    SendLoginAck(SagaLogin.Packets.Server.SSMG_LOGIN_ACK.Result.GAME_SMSG_LOGIN_ERR_ALREADY);
                    return;
                }
                ...
                for (int i = 0; i < charIDs.Length; i++)
                {
                    ...
                    if (pc.QuestNextResetTime < DateTime.Now)
                    {
                        ...
                        else
                        {
                            ...
                            if (pc.QuestRemaining > 15)//ECOKEY 選角色畫面任務點顯示異常的問題
            ...
            {
                // 密碼錯誤處理
                SendLoginAck(SagaLogin.Packets.Server.SSMG_LOGIN_ACK.Result.GAME_SMSG_LOGIN_ERR_BADPASS);
            }
        ...
        // 輔助方法：快速傳送登入回應
        private void SendLoginAck(SagaLogin.Packets.Server.SSMG_LOGIN_ACK.Result result)
        {
            Packets.Server.SSMG_LOGIN_ACK ack = new SagaLogin.Packets.Server.SSMG_LOGIN_ACK();
            ack.LoginResult = result;
            this.netIO.SendPacket(ack);
        }
        ...
        /*  public void OnLogin(Packets.Client.CSMG_LOGIN p)
          {
              ...
              if (LoginServer.accountDB.CheckPassword(p.UserName, p.Password, this.frontWord, this.backWord))
              {
                  ...
                  {
                      Packets.Server.SSMG_LOGIN_ACK p3 = new SagaLogin.Packets.Server.SSMG_LOGIN_ACK();
                      p3.LoginResult = SagaLogin.Packets.Server.SSMG_LOGIN_ACK.Result.GAME_SMSG_LOGIN_ERR_UNKNOWN_ACC;
                      this.netIO.SendPacket(p3);
                      return;
                  }
                  // 檢查帳號是否包含\
        ...
        */
            ...
            else
            {
                ...
                else
                {
                   ...
                   /* if (pc.Race == PC_RACE.DEM)
                    ...
                    pc.Slot = p.Slot;
                    pc.Wig = 0xFF;
                    pc.BaseHairColor = pc.HairColor;
                    pc.BaseHairStyle = pc.HairStyle;
                    pc.BaseWig = pc.Wig;
                    pc.BaseFace = pc.Face;

                    pc.MirrorFaceInfo = new List<ushort>(20) { pc.BaseFace, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF };
                    pc.MirrorHairInfo = new List<ushort>(20) { pc.BaseHairStyle, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF };
                    pc.MirrorHairColor = new List<byte>(20) { pc.BaseHairColor, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
                    pc.MirrorWigInfo = new List<ushort>(20) { pc.BaseWig, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF, 0xFFFF };
        ...
        public void OnCharSelect(Packets.Client.CSMG_CHAR_SELECT p)
        {
            ...
            var chr =
                ...
                select c;
            ActorPC pc = chr.First();
            LoginServer.charDB.GetDualJobInfo(pc);
            selectedChar = pc;
